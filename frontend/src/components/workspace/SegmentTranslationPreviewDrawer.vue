...
import { getQualityCodeLabel, formatQualityIssueTooltip } from '@/composables/useQualityIssues'
...
const startPreview = async (): Promise<void> => {
  ...
  try {
    const result = await previewResourceSegmentTranslation(
      props.projectId,
      resourceId.value,
      segment.value.id,
    ...
    retryAfterSeconds.value = isSegmentTranslationPreviewError(error)
      ...
      : null
    errorMessage.value =
      error instanceof Error ? error.message : t('api.errors.previewSegmentTranslationFailed')
    state.value = 'failed'
  } finally {
...
const handleApply = async (): Promise<void> => {
  ...
  try {
    ...
    state.value = 'applied'
    message.success(t('workspace.segment.translationPreview.applySuccess'))
...
const requestClose = (): void => {
  ...
  dialog.warning({
    title: t('workspace.segment.translationPreview.closeTitle'),
    content: t('workspace.segment.translationPreview.closeConfirm'),
    ...
    >
      <div v-if="segment" class="space-y-4 pb-4">
        ...
        <NAlert v-if="state === 'previewing'" type="info" :bordered="false">
          {{ t('workspace.segment.translationPreview.previewing') }}
        </NAlert>
        <NAlert v-if="stalePlan" type="warning" :bordered="false">
          {{ t('workspace.segment.translationPreview.stalePlan') }}
        </NAlert>
        <NAlert v-if="errorMessage" type="error" :bordered="false">
          ...
          <div v-if="errorStatus === 409" class="mt-1 text-sm">
            {{ t('workspace.segment.translationPreview.conflict') }}
          </div>
        ...
        <template v-if="preview">
          ...
          <NAlert v-else type="error" :bordered="false">
            {{ t('workspace.segment.translationPreview.failed') }}
          </NAlert>
          ...
          <section>
            <div class="mb-1.5 text-xs font-medium text-lf-text-muted">
              {{ t('workspace.segment.translationPreview.result') }}
            </div>
            <NInput
              v-model:value="draftTargetText"
              type="textarea"
              :autosize="{ minRows: 5, maxRows: 14 }"
              :disabled="state === 'applying' || state === 'applied' || preview.status === 'failed'"
            />
          </section>
        ...
        <NEmpty
          ...
          :description="t('workspace.segment.translationPreview.empty')"
        />
      </div>
