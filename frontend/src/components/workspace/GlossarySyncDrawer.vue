...
const resourceColumns = computed<DataTableColumns<SyncImpactResource>>(() => [
  ...
  },
  {
    ...
    align: 'right',
    render: (row) =>
      h('span', { class: 'text-lf-warning tabular-nums' }, String(row.affected_count)),
  },
])
...
const resultColumns = computed<DataTableColumns<SyncExecuteResourceResult>>(() => [
  ...
  },
  {
    ...
    align: 'right',
    render: (row) =>
      h('span', { class: 'text-lf-success tabular-nums' }, String(row.updated_count)),
  },
  {
    ...
    align: 'right',
    render: (row) =>
      h('span', { class: 'text-lf-text-subtle tabular-nums' }, String(row.skipped_count)),
  },
])
...
<template>
  <NDrawer
    ...
    placement="right"
    :mask-closable="false"
    :close-on-esc="glossary.syncStep !== 'executing'"
  >
    <NDrawerContent :closable="glossary.syncStep !== 'executing'" :native-scrollbar="false">
      <template #header>
        <DrawerHeader
          :title="drawerTitle"
          :subtitle="t('workspace.glossary.sync.drawerSubtitle')"
        />
      </template>
        ...
        <template v-else-if="glossary.syncImpactData">
          ...
          <div class="mb-4 rounded-lf-ctl border border-lf-border-soft bg-lf-surface-muted/60 p-4">
            <div class="mb-2 text-sm text-lf-text-muted">
              {{
                t('workspace.glossary.sync.targetChangedWithSource', {
                  source: glossary.syncSource,
                })
              }}
            </div>
      ...
      <div v-if="glossary.syncStep === 'result'" class="py-4">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
          <div class="lf-metric">
            <div class="lf-metric-label">{{ t('workspace.glossary.sync.updated') }}</div>
            <div class="lf-metric-value">{{ glossary.syncResult?.total_updated ?? 0 }}</div>
          </div>
          <div class="lf-metric">
            <div class="lf-metric-label">{{ t('workspace.glossary.sync.skipped') }}</div>
            <div class="lf-metric-value">{{ glossary.syncResult?.total_skipped ?? 0 }}</div>
          </div>
        </div>
      ...
      <div v-if="glossary.syncStep === 'cancelled'" class="py-4">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
          <div class="lf-metric">
            <div class="lf-metric-label">{{ t('workspace.glossary.sync.processed') }}</div>
            <div class="lf-metric-value">{{ glossary.syncProcessed }}</div>
          </div>
          <div class="lf-metric">
            <div class="lf-metric-label">{{ t('workspace.glossary.sync.unprocessed') }}</div>
            <div class="lf-metric-value">{{ glossary.syncTotal - glossary.syncProcessed }}</div>
          </div>
        </div>
