  ...
  <div class="space-y-1.5">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-1">
        ...
        <NButton
          v-if="formatted && formatted.valid"
          quaternary
          size="tiny"
          @click="toggleRaw"
        >
          {{ showRaw ? t('workspace.job.events.batch.showFormatted') : t('workspace.job.events.batch.showRaw') }}
        </NButton>
        <NButton quaternary size="tiny" @click="handleCopy">
          ...
          {{ copySuccess ? t('workspace.job.events.batch.copySuccess') : t('workspace.job.events.batch.copy') }}
        </NButton>
      </div>
    </div>
    <div
      v-if="content"
      class="max-h-60 overflow-auto rounded-lg bg-lf-code-bg p-3"
    >
      <pre class="text-xs text-lf-text whitespace-pre-wrap break-all"><code>{{ displayContent }}</code></pre>
    </div>
