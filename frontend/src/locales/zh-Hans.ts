const messages = {
  common: {
    appName: 'LinguaFlow',
    language: '语言',
    loadMore: '加载更多',
    theme: '主题',
    cancel: '取消',
    confirm: '确定',
    delete: '删除',
    download: '下载',
    save: '保存',
    close: '关闭',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    noDate: '暂无时间',
    actions: {
      refresh: '刷新',
      edit: '编辑',
      view: '查看',
      delete: '删除',
      more: '更多',
      confirmDelete: '确认删除',
      deleteConfirmAction: '删除',
    },
    actionsColumn: '操作',
  },
  nav: {
    main: '主导航',
    menu: '菜单',
    dashboard: '工作台',
    projects: '项目',
    backends: 'AI 后端',
    executionConfig: '模板与配置',
    promptTemplates: '翻译提示词',
    bootstrapPromptTemplates: '术语抽取提示词',
    prunePromptTemplates: '术语精简提示词',
    executionProfiles: '执行策略',
    executionPlanTemplates: '执行计划',
    tools: '工具',
    epubRotate: 'EPUB 横竖排转换',
    stats: '用量概览',
    changelog: '更新日志',
    about: '关于',
    admin: '管理后台',
    adminDashboard: '系统概览',
    adminUsers: '用户管理',
    adminAuditLogs: '审计日志',
    adminSettings: '系统设置',
  },
  epubRotate: {
    title: 'EPUB 横竖排转换',
    description:
      '在浏览器中将整本 EPUB 统一为横排或竖排。即使书内混用横竖排，也会整本统一为目标方向；文件仅在本地处理，不会上传服务器',
    selectFile: '选择 EPUB 文件',
    selectFileHint: '仅支持 .epub；文件不会离开本机',
    selectedFile: '已选择：{name}',
    modeLabel: '转换方式',
    modeHint:
      '始终整书统一为同一方向。指定横排/竖排时全书强制为目标方向；自动则先识别多数方向，再翻转到另一侧',
    modeAuto: '自动翻转',
    detectResult: '转换前内容页：竖排约 {vertical}、横排约 {horizontal}',
    convert: '开始转换',
    convertToVertical: '转为竖排',
    convertToHorizontal: '转为横排',
    targetVertical: '竖排',
    targetHorizontal: '横排',
    unifiedTarget: '结果：全书已统一为{target}',
    processing: '处理中…',
    stageParsing: '正在解析 EPUB…',
    stageConverting: '正在处理（{current}/{total}）…',
    stagePacking: '正在打包…',
    stageDone: '转换完成',
    downloadReady: '转换完成，已开始下载',
    download: '再次下载',
    restart: '用当前选项再转一次',
    selectNew: '换一个文件',
    stats:
      '已处理内容 {content} 个、样式 {styles} 个；{changed} 个方向有变化，{already} 个已是目标方向；{skipped} 个无法处理',
    privacyNote: '文件仅在浏览器内存中处理，不会上传至服务器',
    errors: {
      noFile: '请先选择 EPUB 文件',
      notEpub: '请选择有效的 .epub 文件',
      parseFailed: '无法解析该 EPUB，文件可能已损坏',
      noContent: '未找到可转换的正文或样式',
      tooLarge: '文件过大或压缩结构异常，已中止处理',
      convertFailed: '转换失败，请重试',
    },
  },
  quickTranslate: {
    sourceLabel: '原文',
    sourcePlaceholder: '粘贴需要翻译的文本…',
    charCount: '{count} 字',
    sourceLangLabel: '源语言',
    targetLangLabel: '目标语言',
    swapLangs: '交换源语言与目标语言',
    executionPlanLabel: '执行计划',
    executionPlanPlaceholder: '选择执行计划',
    executionPlanEmpty: '暂无可用执行计划，请先在「执行计划」中创建',
    noTranslatablePlan: '现有执行计划均不含翻译轮次，请先在「执行计划」中配置翻译轮次',
    planNoTranslateRound: '（不含翻译轮次）',
    advancedToggle: '高级选项',
    projectLabel: '项目（可选）',
    projectPlaceholder: '复用项目术语表与语言配置',
    glossaryTitle: '内联术语表',
    glossaryHint: '仅保存在内存中，翻译结束后即丢弃；选择项目时叠加在项目术语表之上',
    glossaryAdd: '添加术语',
    glossarySourcePlaceholder: '源术语',
    glossaryTargetPlaceholder: '目标术语',
    glossaryNotesPlaceholder: '备注（可选）',
    glossaryRemove: '移除',
    submit: '翻译',
    submitting: '翻译中…',
    copy: '复制译文',
    copySuccess: '已复制译文',
    copyFailed: '复制失败，请手动选择复制',
    langPair: '{source} → {target}',
    qualityIssuesTitle: '质量问题',
    roundSummaryTitle: '轮次概览',
    usageTitle: '用量',
    usageApiCalls: 'API 调用',
    usageInputTokens: '输入 Token',
    usageOutputTokens: '输出 Token',
    warningsTitle: '警告',
    batchesTitle: '诊断批次',
    statusSuccess: '成功',
    statusPartial: '部分成功',
    statusFailed: '失败',
    statusSkipped: '已跳过',
    messages: {
      success: '翻译完成',
      partial: '翻译完成，但存在部分问题',
      failed: '翻译失败',
    },
    validation: {
      sourceRequired: '请输入原文',
      planRequired: '请选择执行计划',
    },
  },
  layout: {
    localModeBadge: '本地模式',
    sidebar: {
      collapse: '收起侧栏',
      expand: '展开侧栏',
    },
    userMenu: {
      switchService: '切换服务器',
      connectRemoteService: '连接远程服务',
      logout: '退出登录',
    },
    messages: {
      logoutSuccess: '已退出登录',
      logoutFailed: '退出登录失败，请重试',
    },
  },
  appBootstrap: {
    errors: {
      localUserFailed: '无法获取本地用户信息，请确认本地服务已启动',
      modeUnreachable: '无法获取服务器运行模式',
    },
  },
  secureContext: {
    /** 各能力的用户可读名称（受影响功能清单会拼出这些） */
    capabilities: {
      fileUpload: '文件上传',
      clipboard: '复制到剪贴板',
    },
    /** 启动期一次性提醒标题 */
    bootTitle: '当前为非安全连接，部分功能不可用',
    /** 启动期正文，{features} 为受影响功能清单 */
    bootBody:
      '当前页面通过 HTTP 访问，浏览器限制了以下功能：{features}。请通过 HTTPS 访问，或按下方指引将当前地址标记为可信来源后刷新页面。',
    /** 操作被拦截时的简短提示（完整指引已在启动期给出） */
    uploadBlockedHint: '当前为非安全连接，文件上传不可用',
    clipboardBlockedHint: '当前为非安全连接，复制到剪贴板不可用',
    /** Chrome 指引 */
    chromeGuide:
      'Chrome：地址栏进入 chrome://flags/#unsafely-treat-insecure-origin-as-secure，填入 {origin} 并启用，然后重启浏览器。',
    /** Edge 指引 */
    edgeGuide:
      'Edge：地址栏进入 edge://flags/#unsafely-treat-insecure-origin-as-secure，填入 {origin} 并启用，然后重启浏览器。',
    /** 通用 HTTPS 建议 */
    httpsHint: '生产环境建议通过 HTTPS 提供服务。',
  },
  locale: {
    zhHans: '简体中文',
  },
  theme: {
    system: '跟随系统',
    light: '亮色',
    dark: '暗色',
  },
  service: {
    title: '选择 LinguaFlow 服务器',
    subtitle: '填写要连接的后端 API 地址，支持自部署实例或托管服务',
    form: {
      baseUrl: '服务器地址',
      baseUrlPlaceholder: 'https://linguaflow.example.com/api/v1',
      submit: '连接',
    },
    validation: {
      required: '请填写服务器地址',
      invalidUrl: '请填写合法的 URL，例如 https://linguaflow.example.com/api/v1',
    },
    messages: {
      connected: '已连接到 {name}',
      connectFailed: '连接失败，请检查服务器地址',
      corsOrUnreachable: '无法连接本机服务：请检查地址与端口，或配置开发代理以避免跨域限制',
    },
    hints: {
      prefix: '留空或填写',
      suffix: '将使用当前页面的同源地址',
    },
  },
  login: {
    title: '登录',
    subtitle: '使用账号登录 LinguaFlow',
    form: {
      username: '用户名',
      usernamePlaceholder: '请输入用户名',
      password: '密码',
      passwordPlaceholder: '请输入密码',
      submit: '登录',
    },
    validation: {
      usernameRequired: '请输入用户名',
      passwordRequired: '请输入密码',
    },
    messages: {
      success: '登录成功',
      failed: '登录失败，请检查用户名和密码',
    },
    links: {
      switchService: '切换服务器 · {name}',
      register: '还没有账号？立即注册',
    },
  },
  register: {
    title: '注册账号',
    subtitle: '创建一个 LinguaFlow 账号',
    form: {
      username: '用户名',
      usernamePlaceholder: '3-32 位，字母 / 数字 / 下划线',
      email: '邮箱',
      emailPlaceholder: "you{'@'}example.com",
      displayName: '显示名（可选）',
      displayNamePlaceholder: '留空则使用用户名',
      password: '密码',
      passwordPlaceholder: '至少 8 位',
      confirmPassword: '确认密码',
      confirmPasswordPlaceholder: '再次输入密码',
      submit: '注册并登录',
    },
    validation: {
      passwordMismatch: '两次输入的密码不一致',
      usernameRequired: '请输入用户名',
      usernameLength: '用户名长度需在 3-32 之间',
      emailRequired: '请输入邮箱',
      emailInvalid: '请输入合法的邮箱地址',
      passwordRequired: '请输入密码',
      passwordMinLength: '密码至少 8 位',
      confirmPasswordRequired: '请再次输入密码',
    },
    messages: {
      success: '注册成功，欢迎使用 LinguaFlow',
      failed: '注册失败，请稍后重试',
    },
    links: {
      hasAccount: '已有账号？',
      login: '立即登录',
    },
  },
  dashboard: {
    title: '开始翻译',
    subtitle: '粘贴文本即可获得即时译文，无需创建项目',
    stats: {
      apiCalls: 'API 调用',
      inputTokens: '输入 Token',
      outputTokens: '输出 Token',
      segmentCount: '任务段数',
    },
    quickActions: {
      createProject: {
        title: '新建项目',
        description: '创建专业翻译项目',
      },
    },
    activity: {
      title: '最近活动',
      empty: '暂无活动记录',
      actions: {
        create: '创建了',
        update: '更新了',
        delete: '删除了',
        complete: '完成了',
        fail: '失败了',
        approve: '批准了',
        reject: '拒绝了',
      },
      relativeTime: {
        justNow: '刚刚',
        minutesAgo: '{count} 分钟前',
        hoursAgo: '{count} 小时前',
        daysAgo: '{count} 天前',
      },
    },
    jobStatus: {
      title: '任务状态概览',
      total: '总计 {count} 个任务',
      successRate: '{percent}% 成功率',
      completed: '已完成',
      failed: '失败',
    },
  },
  stats: {
    title: '用量概览',
    subtitle: '查看调用量、Token 消耗与最近活动',
    loadFailed: '加载失败',
  },
  projects: {
    title: '翻译项目',
    subtitle: '管理项目的资源、段落、任务与术语表，跟踪整体翻译进度',
    actions: {
      create: '新建项目',
      createFirst: '创建第一个项目',
      createTitle: '新建翻译项目',
      editTitle: '编辑翻译项目',
      submitCreate: '创建项目',
      submitUpdate: '保存',
    },
    filters: {
      all: '全部',
      glossaryEnabled: '术语表启用',
      glossaryDisabled: '术语表停用',
      searchPlaceholder: '搜索项目名称或语言代码',
      reset: '重置筛选',
    },
    languages: {
      auto: '自动识别 · auto',
      en: '英语 · en',
      zhHans: '简体中文 · zh-Hans',
      zhHant: '繁体中文 · zh-Hant',
      es: '西班牙语 · es',
      fr: '法语 · fr',
      de: '德语 · de',
      pt: '葡萄牙语 · pt',
      ja: '日语 · ja',
      ko: '韩语 · ko',
      ar: '阿拉伯语 · ar',
      ru: '俄语 · ru',
      it: '意大利语 · it',
    },
    card: {
      updatedAt: '更新于',
    },
    empty: {
      default: '暂无项目，创建一个开始管理翻译流程',
      filtered: '没有找到符合条件的项目',
    },
    delete: {
      confirm: '确定要删除项目「{name}」吗？此操作无法撤销。',
    },
    form: {
      name: '项目名称',
      namePlaceholder: '例如：LinguaFlow 本地化',
      sourceLang: '源语言',
      targetLang: '目标语言',
      languagePlaceholder: '选择或输入语言代码',
      glossaryEnabled: '术语表已启用',
      glossaryDisabled: '术语表未启用',
      glossaryToggle: '启用术语表',
      createHint: '填写名称与语言方向，创建后可在工作区内上传资源并创建翻译任务',
    },
    validation: {
      nameRequired: '请输入项目名称',
      sourceLangRequired: '请选择或输入源语言',
      targetLangRequired: '请选择或输入目标语言',
    },
    messages: {
      createSuccess: '项目已创建',
      createFailed: '项目创建失败，请稍后重试',
      updateSuccess: '项目已更新',
      updateFailed: '项目更新失败，请稍后重试',
      deleteSuccess: '项目已删除',
      deleteFailed: '项目删除失败，请稍后重试',
    },
  },
  workspace: {
    loadingProject: '正在加载项目',
    updatedAt: '更新于 {time}',
    editor: {
      backToBrowse: '返回工作台',
      progressLabel: '已翻译 {percent}%',
      collapsePanel: '收起（文档区宽度不受影响）',
    },
    actions: {
      clear: '清除',
    },
    tabs: {
      resources: '资源文件',
      jobs: '任务',
      glossary: '术语表',
    },
    stats: {
      resources: '资源文件',
      segments: '段落总数',
      runningJobs: '运行中任务',
      progress: '已翻译',
    },
    selection: {
      translate: '处理选中',
      approve: '批准',
      reject: '拒绝',
      clear: '取消选择',
      qaRecheck: 'QA 重检',
    },
    sections: {
      resources: {
        title: '资源文件',
        description: '管理项目源文件，可上传、替换或进入段落编辑',
      },
      segments: {
        title: '段落编辑',
        description: '选择资源后检索原文、译文与翻译状态',
      },
      jobs: {
        title: '任务',
        description: '跟踪任务进度，查看执行详情',
      },
      glossary: {
        title: '术语表',
        description: '管理项目翻译术语，确保关键术语在翻译过程中保持一致',
      },
    },
    filters: {
      allStatuses: '全部状态',
      qualityLabel: '质量',
      qualityIssuesHas: '有问题',
      qualityIssuesNone: '无问题',
      clearQuality: '清除',
    },
    explorer: {
      rootLabel: '项目根目录',
      emptyDirectory: '当前目录为空，上传文件后即可开始翻译',
      dropToUpload: '拖拽文件或文件夹到此处上传',
      dropHint: '也可以直接把文件或文件夹拖入此区域，系统会保留当前目录层级',
      backToParent: '返回上级',
      refreshDirectory: '刷新目录',
      childCount: '{count} 项',
      currentDirectorySummary: '{directoryCount} 个文件夹 / {fileCount} 个文件',
      headerName: '名称',
      headerSegments: '段落',
      headerProgress: '进度',
      selectCurrentDirectoryResources: '选择当前目录及所有子目录中的资源',
      deselectCurrentDirectoryResources: '取消选择当前目录及所有子目录中的资源',
      selectResource: '选择资源',
      deselectResource: '取消选择资源',
      selectDirectoryResources: '选择文件夹「{name}」中的所有资源',
      deselectDirectoryResources: '取消选择文件夹「{name}」中的所有资源',
    },
    resource: {
      searchPlaceholder: '搜索资源路径或文件名',
      empty: '暂无资源文件，上传文件后即可解析段落并创建任务',
      deleteConfirm: '确定要删除资源「{name}」吗？相关段落将一并删除。',
      columns: {
        name: '文件名',
        segments: '段落数',
      },
      actions: {
        upload: '上传资源',
        uploadFirst: '上传第一个资源',
        replace: '替换',
        replacing: '正在替换…',
        incrementalUpdate: '增量更新',
        incrementalUpdating: '正在增量更新…',
        downloading: '正在下载…',
        downloadTranslated: '下载译文',
        downloadingTranslated: '正在下载译文…',
        segments: '查看段落',
      },
    },
    upload: {
      batchName: '{count} 个文件',
    },
    uploadPanel: {
      clearAll: '清除全部',
      statusPrechecking: '正在预检 {count} 个文件…',
      statusUploading: '正在上传 {count} 个文件… {percent}%',
      statusProcessing: '服务端正在处理 {count} 个文件…',
      statusComplete: '{count} 个文件上传完成',
      statusPartial: '{count} 个文件部分完成',
      statusError: '{count} 个文件上传失败',
      stageLabels: {
        prechecking: '预检中',
        uploading: '上传中',
        processing: '处理中',
        complete: '已完成',
        partial: '部分完成',
        error: '失败',
      },
    },
    uploadPrecheck: {
      drawerTitle: '上传预检',
      drawerSubtitle: '确认各文件的处理方式后开始上传',
      title: '确认上传文件',
      descriptionReady: '所有文件均可创建，将直接上传所选文件',
      descriptionWithProblems: '检测到冲突或重复路径，冲突文件默认增量更新，也可切换为覆盖或跳过',
      selectedHint: '将处理 {count} 项；跳过项不会写入项目',
      confirmSelected: '开始处理 {count} 项',
      columns: {
        selectAllCreatable: '批量选择可创建资源',
        conflictResolution: '冲突处理',
      },
      summary: {
        creatable: '新建',
        conflicts: '冲突',
        duplicates: '重复',
        incrementalUpdates: '增量更新',
        replaces: '覆盖',
      },
      actions: {
        create: '可创建',
        conflict: '冲突',
        duplicate: '重复',
      },
      strategies: {
        create: '将上传',
        createHint: '新资源将按当前路径创建',
        incrementalUpdate: '增量更新',
        incrementalUpdateHint: '保留未变化段落的已有译文，仅更新变更内容',
        replace: '覆盖',
        replaceHint: '使用新文件完全替换已有资源，已有译文将重新生成',
        skip: '跳过',
        skipHint: '本次不处理该文件',
      },
      reasons: {
        create: '路径可用，将创建新资源',
        conflict: '已存在资源「{name}」，请选择增量更新、覆盖或跳过',
        duplicate: '本批次存在相同路径，请保留一个有效文件，其余跳过',
      },
    },
    uploadResult: {
      summary: {
        created: '已创建',
        incrementallyUpdated: '已增量更新',
        replaced: '已覆盖',
        conflicts: '冲突',
        failed: '失败',
        skipped: '已跳过',
      },
      details: {
        created: '资源已创建并完成解析',
        conflict: '已存在同名资源「{name}」，请选择增量更新、覆盖或跳过',
        incrementalUpdated: '资源已增量更新，未变化段落的译文已保留',
        replaced: '资源已被新文件覆盖并重新解析',
        failed: '服务端处理失败',
        missingExistingResource: '缺少已有资源信息，无法执行冲突处理',
        skipped: '已选择跳过',
      },
    },
    segment: {
      resourcePlaceholder: '选择资源文件',
      chapterAll: '全部章节',
      chapterPlaceholder: '选择章节…',
      chapterSidebarTitle: '章节',
      chapterProgress: '已译 {translated} / {count}',
      batchSelect: '批量处理',
      batchSelectedCount: '已选 {count} 章',
      batchSelectAll: '全选',
      batchClear: '清空',
      batchExit: '退出',
      batchTranslate: '处理选中',
      searchLocateToggle: '搜索',
      searchLocateToggleHint: '搜索定位（Ctrl+F）',
      openChapters: '章节',
      searchPlaceholder: '搜索原文或译文',
      searchFieldBoth: '原文+译文',
      searchFieldSource: '仅原文',
      searchFieldTarget: '仅译文',
      searchCaseSensitive: '区分大小写',
      resultCount: '已加载 {shown} / 共 {total} 段',
      searchLocate: {
        title: '搜索定位',
        placeholder: '搜索全部章节的原文 / 译文…',
        stats: '{count} 个结果 · {chapters} 个章节',
        loading: '搜索中…',
        emptyHint: '搜索结果会显示章节位置与上下文片段，点击任意结果跳转到该段在列表中的位置。',
        noResults: '没有找到包含 "{query}" 的段落',
        jump: '跳转',
        hitSource: '命中 原文',
        hitTarget: '命中 译文',
        hitBoth: '命中 原文+译文',
        hitGeneric: '命中',
        caseSensitive: '区分大小写',
        matchMode: '匹配模式',
        matchModeSubstring: '子串',
        matchModeRegex: '正则',
        matchModeSubstringHint: '当前为字面子串匹配，点击切换为正则表达式',
        matchModeRegexHint: '当前为正则表达式匹配，点击切换为字面子串',
        wholeWord: '全字',
        wholeWordHint: '仅匹配完整词汇：命中前后不紧邻字母或数字',
        searchField: '搜索字段',
        searchFieldBoth: '原文+译文',
        searchFieldSource: '仅原文',
        searchFieldTarget: '仅译文',
        clear: '清空',
        keyboardHint: '↑↓ 选择 · Enter 跳转',
        noAffect: '搜索不改变主列表',
        loadMoreHint: '滚动加载更多结果',
        retryLoad: '重试加载',
        replaceMode: '替换',
      },
      noResource: '请先选择一个资源文件查看段落',
      empty: '当前资源没有符合条件的段落',
      emptyTarget: '暂无译文',
      qualityError: '错误',
      qualityWarning: '警告',
      qualityMatched: '片段：{text}',
      qualityCodes: {
        untranslated: '未翻译',
        lengthRatio: '长度异常',
        duplicate: '重复译文',
        sourceResidual: '源文残留',
        calque: '仿译',
        termFidelity: '术语偏差',
        naturalness: '自然度',
        mistranslation: '误译',
        omission: '漏译',
        addition: '增译',
        grammar: '语法',
        register: '语域',
        punctuationPairing: '标点配对',
        punctuationMissing: '标点缺失',
        punctuationSurplus: '标点多余',
        punctuationWrapLoss: '引号包裹缺失',
        whitespaceIrregular: '空白异常',
        repeatedSpace: '重复空格',
        widthMix: '全半角混用',
        scriptMismatch: '文字系统混用',
        numberMismatch: '数字不一致',
        urlEmailMismatch: '链接邮箱不一致',
        subtitleLineCount: '字幕行数',
        forbiddenTerm: '禁译词',
        termInconsistency: '术语不一致',
        leftoverPlaceholder: '占位符残留',
        xmlTagMismatch: 'XML 标签不一致',
        rubyRestoreIncomplete: '注音还原不完整',
        rubyTagLoss: '注音标签丢失',
        duplicateSourceDivergence: '同源译文偏差',
      },
      disposition: {
        dismissed: '已忽略',
        dismissAction: '判定不是问题',
        reinstateAction: '撤销裁决',
        dismissTitle: '忽略质量问题',
        reinstateTitle: '撤销忽略',
        noteLabel: '裁决说明',
        notePlaceholder: '可选，如「专有名词，有意保留」',
        confirmDismiss: '确定忽略',
        confirmReinstate: '确定撤销',
        cancel: '取消',
        dismissSuccess: '已忽略该质量问题',
        reinstateSuccess: '已撤销裁决',
        dismissedHint: '已判定为非问题',
        decidedByLlm: 'LLM 裁决',
        decidedByUser: '由用户 {userId} 裁决',
        decidedAt: '裁决时间：{time}',
      },
      qualityCodeGroups: {
        hardRule: '硬规则',
        typography: '排版标点',
        content: '数字链接',
        markup: '占位标签',
        term: '术语',
        semantic: '语义质量',
        subtitle: '字幕',
        divergence: '同源偏差',
      },
      qualityCodePlaceholder: '按问题类型筛选',
      editTitle: '编辑段落',
      previewSubtitle: '对比原文与译文并检查质量问题，确认无误后应用结果',
      columns: {
        source: '原文',
        target: '译文',
        status: '状态',
        qualityIssues: '质量',
      },
      actions: {
        edit: '编辑',
        translate: '翻译',
        previewTranslation: '单段试译',
        previewRevision: '单段修订',
        translateSelected: '处理选中 ({count})',
        saveInline: '保存',
        saveAndNext: '保存并下一个',
        cancelInline: '取消',
        comment: '备注',
        viewSourceHtml: '查看 HTML 源码',
        hideSourceHtml: '隐藏 HTML 源码',
      },
      form: {
        target: '译文',
        comment: '备注',
      },
      status: {
        pending: '待翻译',
        translated: '已翻译',
        edited: '已编辑',
        approved: '已批准',
        rejected: '已拒绝',
      },
      searchReplace: {
        title: '搜索替换',
        subtitle: '批量替换当前资源的译文，支持预览影响范围与撤销',
        findLabel: '查找',
        findPlaceholder: '输入要查找的文本',
        findHint: '仅匹配译文；正则模式支持 $1 捕获引用',
        replaceLabel: '替换为',
        replacePlaceholder: '替换文本（留空表示删除匹配内容）',
        matchModeLabel: '匹配方式',
        modeSubstring: '字面子串',
        modeRegex: '正则',
        caseSensitive: '区分大小写',
        wholeWord: '全字匹配',
        regexHint: '正则使用 RE2 语法（线性时间）；全字匹配在正则模式下忽略，可用 \\b 边界替代',
        wholeWordHint: '仅匹配完整词汇，不含子串命中',
        scopeLabel: '作用范围',
        scopeAll: '全部命中段落',
        scopeSelected: '仅选中段落（{count}）',
        stalePreview: '查找参数或作用范围已变更，请重新预览后再应用',
        preview: '预览',
        clearResults: '清除结果',
        apply: '应用替换',
        undo: '撤销上次替换',
        matchedSegments: ' 个段落受影响',
        totalReplacements: ' 处命中',
        matchCount: '命中 {count} 次',
        samples: '预览样本',
        samplesTruncated: '仅显示前 {shown} 条样本，应用时作用于范围内全部命中段落',
        before: '替换前',
        after: '替换后',
        applyConfirmTitle: '应用搜索替换',
        applyConfirmContent:
          '将对 {segments} 个段落执行共 {replacements} 处替换，替换后段落状态会标记为已编辑。确定继续吗？',
        applySuccess: '已替换 {applied} 个段落，跳过 {skipped} 个',
        applyResultTitle: '应用结果',
        appliedCount: '已应用 {count} 个段落',
        skippedCount: '跳过 {count} 个段落',
        undoConfirmTitle: '撤销搜索替换',
        undoConfirmContent:
          '将把上次替换（或撤销）涉及的段落回滚到替换前的译文；已被后续编辑的段落将被跳过。确定继续吗？',
        undoSuccess: '已撤销 {undone} 个段落，跳过 {skipped} 个',
        undoNotFound: '该替换历史不存在或已被清理，无法撤销',
        undoConflict: '所有相关段落均已被后续编辑改变，无可撤销内容',
        empty: '输入查找内容后点击「预览」查看影响范围',
        skipReasons: {
          no_longer_matches: '当前译文已不含匹配内容',
          empty_result: '替换后译文为空，已跳过',
          target_diverged: '该段落已被后续编辑，已跳过',
          invalid_markup: '替换后译文标签结构非法，已跳过（否则导出时整章降级为原文）',
        },
      },
      translationPreview: {
        title: '单段试译',
        source: '原文',
        currentTarget: '当前译文',
        result: '试译结果',
        planLabel: '执行计划',
        planPlaceholder: '选择包含翻译轮次的执行计划',
        start: '开始试译',
        retry: '重新试译',
        apply: '应用译文',
        applyPartial: '仍然应用',
        previewing: '正在生成试译预览，请稍候…',
        empty: '选择执行计划后开始试译',
        success: '试译完成',
        partial: '部分成功',
        failed: '试译失败',
        partialWarning: '试译已生成译文，但部分轮次存在警告或失败，请确认后再应用',
        stalePlan: '已更换执行计划，当前结果属于旧计划，请重新试译后再应用',
        tokenExpired: '应用凭证已过期，请重新试译',
        expiresAt: '应用凭证有效至 {time}',
        qualityIssues: '质量问题',
        diagnostics: '诊断详情',
        applied: '译文已应用',
        applySuccess: '试译译文已应用',
        closeTitle: '关闭试译',
        closeConfirm:
          '预览正在执行。关闭后会停止等待，但后端可能仍继续执行并产生用量。确定关闭吗？',
        rateLimited: '当前预览并发已满，请稍后重试',
        rateLimitedWithRetry: '当前预览并发已满，请在 {seconds} 秒后重试',
        conflict: '段落内容或状态已变化，旧预览已失效，请关闭后刷新或重新打开试译',
        executionSummary: '{name}，共 {rounds} 轮',
        status: {
          success: '成功',
          partial: '部分成功',
          failed: '失败',
        },
        stage: {
          extract: '术语抽取',
          translate: '翻译',
          adjudicate: '质量裁决',
          semantic_qa: '语义质检',
          revise: 'LLM 修订',
          ruby_retry: '注音重试',
        },
        diagnostic: {
          batch: '批次 {index}',
          round: '轮次 {index}',
          attempt: '第 {count} 次尝试',
          duration: '{ms} ms',
          segmentCount: '{count} 段',
          tokens: '{input}↑ {output}↓ Token',
          errorType: '错误类型',
          errorMessage: '错误信息',
          httpStatus: 'HTTP 状态',
          triedBackends: '尝试后端',
          shrinkAttempted: '已尝试缩减请求',
          truncated: '输出被截断',
          truncatedHint:
            '响应因输出 Token 上限被截断，已由修复链抢救有效前缀，缺失条目将走重跑通道',
          repaired: '解析修复 {ops}',
          repairedHint: '响应解析修复算子链，按应用顺序记录',
          expand: '查看详情',
          collapse: '收起详情',
          systemPrompt: 'System Prompt',
          userMessage: 'User Message',
          jsonSchema: 'JSON Schema',
          rawResponse: '原始响应',
        },
      },
      revisionPreview: {
        title: '单段修订',
        originalTarget: '修订前译文',
        planPlaceholder: '选择包含修订或翻译轮次的执行计划',
        issueCodesLabel: '问题代码筛选',
        issueCodesPlaceholder: '不选则修复全部未决语义问题',
        issueCodesHint: '可选：收窄修复目标；与段落实际未决语义问题取交集',
        start: '开始修订',
        retry: '重新修订',
        apply: '应用修订译文',
        previewing: '正在生成修订预览，请稍候…',
        empty: '选择执行计划后开始修订',
        failed: '修订失败，未得到可用译文',
        precondition: '修订要求段落已翻译或已编辑、译文非空，且存在未决（pending）的语义质量问题',
        pendingIssues: '未决语义问题',
        fixIssues: '本次修复目标',
        result: '修订结果',
        unchanged: '修订译文与原译文相同，LLM 判定无需改动',
        usageSummary: '{calls} 次调用 · {input}↑ {output}↓ Token',
        roundItem: '修订轮 {index}',
        synthesizedRound: '默认修订轮（由翻译轮合成）',
        stalePlan: '已更换执行计划或筛选条件，当前结果已过期，请重新修订后再应用',
        conflict: '段落内容或状态已变化，或所选问题代码与未决语义问题无交集，请刷新后重试',
        applySuccess: '修订译文已应用',
        closeTitle: '关闭修订',
        closeConfirm:
          '修订正在执行。关闭后会停止等待，但后端可能仍继续执行并产生用量。确定关闭吗？',
      },
    },
    job: {
      empty: '暂无任务',
      polling: '自动刷新中',
      waitingJobs: '等待任务',
      autoRefresh: '自动刷新',
      legacyPlan: '旧版任务',
      createTitle: '创建任务',
      createSubtitle: '选择执行计划模板与段落过滤策略，为选中的资源或段落创建翻译任务',
      detailTitle: '任务 #{id}',
      detailFallbackTitle: '任务详情',
      detailEmpty: '暂无任务详情',
      resourcesTitle: '资源执行明细',
      targetResources: '将为 {count} 个资源创建任务',
      targetSegments: '将为 {count} 个段落创建任务',
      executionPlanLabel: '{name}（{rounds} 轮）',
      contentSummaryTitle: '内容摘要',
      contentSummaryResources: '{count} 个资源文件',
      contentSummaryChapters: '{count} 个章节',
      contentSummarySegments: '{count} 个段落',
      planPreviewTitle: '执行计划详情',
      planPreviewRounds: '共 {count} 个轮次',
      planPreviewRoundItem: '轮次 {index} · {mode}：{batch}，并发 {concurrency}',
      planPreviewBatchSegments: '段落 {batchSize}',
      planPreviewBatchWords: '字词 {maxWordsPerBatch}',
      planPreviewNoBatch: '不分批',
      planPreviewModeTranslate: '翻译',
      planPreviewModeExtract: '术语抽取',
      planPreviewModeAdjudicate: '质量裁决',
      planPreviewModeSemanticQA: '语义质检',
      planPreviewModeRevise: 'LLM 修订',
      planPreviewModeCorrect: '本地改写',
      confirmSummary:
        '将使用「{planName}」为 {resourceCount} 个资源（{segmentCount} 个段落）创建任务',
      segmentFilter: {
        pendingOnly: '仅待翻译（处理待翻译和已拒绝的段落）',
        skipApproved: '跳过已批准（处理除已批准外的所有段落）',
        all: '全部段落（处理所有状态的段落）',
      },
      columns: {
        id: '任务',
        status: '状态',
        progress: '进度',
        executionPlan: '执行计划',
        resources: '资源',
        workload: '工作量',
        trigger: '触发方式',
        error: '错误信息',
        remark: '备注',
        startedAt: '开始时间',
        duration: '耗时',
        languagePair: '语言方向',
      },
      warnings: {
        badge: '{count} 项警告',
        summary: '{count} 个资源完成时出现软警告（任务状态仍为已完成）',
      },
      actions: {
        create: '创建任务',
        submitCreate: '创建任务',
        details: '详情',
        pause: '暂停',
        resume: '恢复',
        cancel: '取消',
        retry: '重试',
      },
      form: {
        executionPlan: '执行计划模板',
        executionPlanPlaceholder: '选择执行计划模板',
        autoApprove: '翻译完成后自动审核',
        autoApproveHint: '开启后，任务完成时将自动批准所有段落',
        sourceLang: '源语言',
        targetLang: '目标语言',
        segmentFilter: '段落过滤策略',
        segmentFilterOverride: '覆盖执行模板配置',
        segmentFilterFollowHint: '使用执行模板中的段落过滤配置',
        segmentFilterScopeHint: '仅对翻译轮次生效',
      },
      validation: {
        executionPlanRequired: '请选择执行计划模板',
      },
      // 阶段名称（轮次模式标签）
      stage: {
        bootstrap: '术语提取',
        extract: '术语提取',
        translate: '翻译',
        adjudicate: '质量裁决',
        semantic_qa: '语义质检',
        revise: 'LLM 修订',
        correct: '本地改写',
        review: '审校',
        finalize: '收尾',
        ruby_alignment: '注音对齐',
      },
      // 轮次矩阵（资源×轮次）单元格与提示
      round: {
        roundLabel: '轮次 {index}',
        status: {
          pending: '等待中',
          running: '执行中',
          completed: '已完成',
          failed: '失败',
          skipped: '已跳过',
        },
        segments: '{completed}/{total} 段',
        startedAt: '开始 {time}',
        finishedAt: '结束 {time}',
        etaRemaining: '预计剩余 {duration}',
        expectedFinish: '预计完成 {time}',
        legacyHint: '旧版任务无轮次明细',
        skippedHint: '本轮无段可处理',
      },
      // 进度文案
      progress: {
        waiting: '等待中…',
        startingSoon: '即将开始',
        queued: '排队中，前面有 {ahead} 个任务',
        running: '已完成 {completed}/{total} 工作量',
        paused: '已暂停，可随时恢复',
        percentTooltip: '百分比为已知工作量占比；新轮次启动时分母增大，进度可能暂时回落',
        completed: '翻译完成',
        failed: '翻译失败',
        cancelled: '已取消',
        workload: '已完成工作量 {completed}/{total}',
      },
      // ETA 文案
      eta: {
        label: '预计剩余',
        lessThanOneMin: '不到 1 分钟',
        minutes: '约 {count} 分钟',
        hours: '约 {count} 小时',
        hoursMinutes: '约 {hours} 小时 {minutes} 分钟',
        expectedLabel: '预计完成',
        expectedToday: '今天 {time}',
        expectedDate: '{date} {time}',
      },
      // 速度文案
      speed: {
        label: '翻译速度',
        verySlow: '速度较慢',
        perMinute: '{count} 段落/分钟',
      },
      // 任务耗时文案
      duration: {
        seconds: '{count} 秒',
        minutes: '{count} 分钟',
        hours: '{count} 小时',
        hoursMinutes: '{hours} 小时 {minutes} 分',
      },
      // 统计标签
      stats: {
        completed: '已完成',
        skipped: '已跳过',
        failed: '已失败',
        warned: '有警告',
        total: '总计',
        unitWorkload: '段×轮',
        unitSegments: '段落',
        unitResources: '资源',
      },
      // 事件日志
      events: {
        title: '事件日志',
        empty: '暂无事件记录',
        live: '实时',
        offline: '离线',
        connected: 'SSE 已连接',
        disconnected: 'SSE 未连接',
        newEvents: '新事件 ({count})',
        loadingOlder: '加载更早事件…',
        pullToLoad: '下拉加载更早事件',
        releaseToLoad: '松开加载',
        reachedOldest: '已到最早',
        jobEnded: '任务已结束',
        batch: {
          status: {
            success: '成功',
            partial: '部分成功',
            failed: '失败',
          },
          segments: '{count} 段',
          tokens: '{input}↑ {output}↓ Token',
          tokenSpeed: '{rate} tok/s',
          glossaryUsed: '{count} 使用',
          glossaryAdded: '{count} 新增',
          detailTitle: '批次详情',
          detailSubtitle: '查看该批次的请求与响应内容、Token 用量与术语命中明细',
          errorBadge: '错误',
          sentContent: '发送内容',
          receivedContent: '接收内容',
          usedGlossary: '使用术语',
          addedGlossary: '新增术语',
          copy: '复制',
          copySuccess: '已复制',
          showRaw: '原始',
          showFormatted: '格式化',
          errorType: '错误类型',
          errorMessage: '错误信息',
          httpStatus: 'HTTP 状态',
          triedBackends: '尝试后端',
          contentTruncated: '已截断（原始 {length} 字节）',
          shrinkAttempted: '已尝试缩放',
          truncated: '输出被截断',
          truncatedHint:
            '响应因输出 Token 上限被截断，已由修复链抢救有效前缀，缺失条目将走重跑通道',
          repaired: '解析修复 {ops}',
          repairedHint: '响应解析修复算子链，按应用顺序记录',
          noContent: '无内容',
          malformedJson: '内容格式异常，显示原始文本',
        },
        pool: {
          poolStart: '池开始',
          poolAdvance: '缩批推进',
          progress: '池 {index}/{total}',
          batches: '{count} 批',
          pending: '{count} 段待处理',
          shrinkRate: '缩放 {rate}',
        },
      },
      detail: {
        executionPlan: '执行计划',
      },
      status: {
        pending: '等待中',
        running: '运行中',
        paused: '已暂停',
        completed: '已完成',
        failed: '失败',
        cancelled: '已取消',
      },
      statusSubtitle: {
        pending: '任务正在排队，开始执行后进度将实时更新',
        running: '任务正在执行，进度与事件日志实时更新',
        paused: '任务已暂停，可随时恢复执行',
        completed: '任务已执行完成，可查看各资源的执行明细',
        failed: '任务执行失败，可根据错误信息排查后重试',
        cancelled: '任务已取消，已完成部分的结果仍会保留',
      },
      trigger: {
        manual: '手动',
        file_update: '文件更新',
        glossary_change: '术语更新',
        web_edit: '网页编辑',
      },
    },
    conflict: {
      title: '文件冲突',
      description: '已存在同名资源「{name}」',
      hint: '请选择处理方式：全量替换将清除所有已有译文，增量更新将保留未变化段落的译文',
      fullReplace: '全量替换',
      incrementalUpdate: '增量更新',
    },
    incremental: {
      resultTitle: '增量更新完成',
      added: '新增',
      updated: '已修改',
      unchanged: '未变化',
      deleted: '已删除',
    },
    qaRecheck: {
      title: 'QA 重检',
      drawerSubtitle: '按执行策略当前的 QA 配置重跑质量检查，仅更新问题列表，不修改译文',
      action: 'QA 重检',
      intro:
        '按所选执行策略当前的 QA 配置，对既有译文重跑质量检查。仅更新质量问题，不修改译文与段落状态；已有人工裁决（含忽略）按指纹自动继承',
      profileLabel: '执行策略',
      profilePlaceholder: '选择要使用的执行策略',
      profileHint: 'QA 检查项与阈值取自该策略的当前配置；未启用质量检测的策略不可选',
      profileQaDisabled: '未启用 QA',
      noProfiles: '暂无可用的执行策略',
      targetLabel: '重检目标',
      targetResources: '选中的 {count} 个资源',
      targetChapters: '选中的 {count} 个章节',
      targetSegments: '选中的 {count} 个段落',
      scopeLabel: '重检范围',
      scopeProject: '整个项目（全部资源）',
      scopeProjectHint: '对项目内全部资源重跑 QA 检查',
      scopeResource: '当前资源',
      scopeResourceHint: '仅重检当前查看的资源：{name}',
      scopeChapter: '当前章节',
      scopeChapterHint: '仅重检当前查看的章节：{key}',
      scopeSelected: '选中的段落（{count}）',
      scopeSelectedHint: '仅重检段落表格中勾选的 {count} 个段落',
      confirmTitle: '确认执行 QA 重检',
      confirmContent:
        '将使用策略「{profile}」对既有译文重跑质量检查，问题列表会被重写（人工裁决保留）。确定继续吗？',
      submit: '开始重检',
      successToast: 'QA 重检完成',
      resultSummary: '已使用策略「{profile}」完成重检',
      resultSegmentsChecked: '重检段落',
      resultResourcesChecked: '重检资源',
      resultIssuesNew: '新增问题',
      resultIssuesCleared: '已清除问题',
      resultDispositionsInherited: '继承既有裁决 {count} 项（含已忽略）',
      resultSkippedNoTarget: '{count} 个段落因无译文跳过',
      resultSkippedConcurrent: '{count} 个段落因译文被并发修改跳过',
      resultSkippedBusyTitle: '以下资源存在未完成任务，已跳过：',
      resultSkippedBusyItem: '{name}（任务 #{jobId} 运行中）',
      resultResourceDetail: '资源明细',
      resourceRow: '{name}：重检 {checked} 段，新增 {added}，清除 {cleared}',
    },
    glossary: {
      actions: {
        create: '新增条目',
        import: '导入 CSV',
        export: '导出 CSV',
        prune: '智能精简',
      },
      columns: {
        source: '源术语',
        target: '目标译文',
        type: '类型',
        caseSensitive: '区分大小写',
        notes: '备注',
      },
      form: {
        drawerSubtitle: '维护术语的推荐译法与处理规则，翻译时将按条目保持一致',
        source: '源术语',
        sourcePlaceholder: '输入源语言术语',
        target: '目标译文',
        targetPlaceholder: '输入目标语言译文',
        forbidden: '禁译条目',
        forbiddenHint: '命中源词且译文包含目标词时产生错误',
        mandatory: '强制术语',
        mandatoryHint: '推荐译法标记为强制术语；仅影响非禁译条目的严重级',
        caseSensitive: '区分大小写',
        notes: '备注',
        notesPlaceholder: '可选，添加备注说明',
        notesPresetGroups: {
          type: '类型',
          handling: '处理',
        },
        notesPresets: {
          type: {
            personName: '人名',
            maleName: '男性人名',
            femaleName: '女性人名',
            surname: '姓氏',
            characterName: '角色名',
            formOfAddress: '称谓',
            placeName: '地名',
            orgName: '机构名',
            brandName: '品牌名',
            productName: '产品名',
            workTitle: '作品名',
            acronym: '缩写',
          },
          handling: {
            keepOriginal: '保留原文',
            keepEnglish: '保留英文',
            doNotTranslate: '不翻译',
            transliterate: '音译',
            freeTranslate: '意译',
            fixedTranslation: '固定译法',
            allCaps: '全大写',
            capitalize: '首字母大写',
          },
        },
      },
      type: {
        forbidden: '禁译',
        mandatory: '强制',
        normal: '普通',
      },
      validation: {
        sourceRequired: '请输入源术语',
        targetRequired: '请输入目标译文',
      },
      empty: {
        default: '暂无术语条目，添加术语以确保翻译一致性',
        filtered: '没有找到符合条件的术语',
      },
      import: {
        title: '导入术语表',
        description: '上传 CSV 文件，格式为：source,target,case_sensitive,notes',
        result: '导入完成：成功 {added} 条',
        skipped: '{count} 条已跳过：{reasons}',
      },
      messages: {
        createSuccess: '术语条目已创建',
        createFailed: '术语条目创建失败',
        updateSuccess: '术语条目已更新',
        updateFailed: '术语条目更新失败',
        deleteSuccess: '术语条目已删除',
        deleteFailed: '术语条目删除失败',
        importSuccess: '术语表已导入',
        importFailed: '术语表导入失败',
        exportFailed: '术语表导出失败',
      },
      prune: {
        title: '智能精简术语表',
        subtitle: '由 AI 分析术语表并给出删除或更新建议，确认后才会应用',
        description:
          'AI 将分析当前术语表并给出删除或更新建议；预览不会修改数据，仅勾选并确认的建议才会应用',
        backend: 'AI 后端',
        backendPlaceholder: '选择用于分析的 AI 后端',
        backendRequired: '请选择 AI 后端',
        template: '分析模板',
        templateRequired: '请选择分析模板',
        analyze: '生成建议',
        analyzeAgain: '重新分析',
        apply: '应用 {count} 项',
        close: '关闭',
        total: '术语总数',
        toDelete: '建议删除',
        toUpdate: '建议更新',
        toKeep: '建议保留',
        deleted: '已删除',
        updated: '已更新',
        failed: '失败',
        selectedCount: '已选择 {count} 项变更',
        noSuggestions: '术语表状态良好，没有需要应用的精简建议',
        deleteHint: '删除此术语',
        resultSummary: '精简变更已处理，术语表已刷新',
        previewFailed: '生成术语精简建议失败',
        applyFailed: '应用术语精简建议失败',
        applySuccess: '术语精简变更已应用',
        applyPartialSuccess: '术语精简已部分应用，部分条目失败。为避免误改译文，本次未自动打开同步',
        columns: {
          action: '操作',
          source: '源术语',
          current: '当前内容',
          suggestion: '建议内容',
        },
        actions: {
          delete: '删除',
          update: '更新',
        },
        diagnostics: {
          title: '诊断信息',
          noData: '无诊断数据',
          summary: '调用概览',
          analysisFailed: '术语精简分析失败',
          analysisFailedHint: 'LLM 调用未完成，以下为失败诊断信息，请检查后端配置或提示词模板',
          metrics: {
            backend: '后端',
            template: '模板',
            duration: '耗时',
            durationValue: '{ms} ms',
            promptTokens: '输入 Token',
            completionTokens: '输出 Token',
            totalTokens: 'Token 总量',
            totalTokensValue: '{total}',
            httpStatus: 'HTTP 状态',
            entryCount: '条目总数',
            parsedCount: '解析条目',
            repairedOps: '修复操作',
            repairedOpsValue: '{count} 项',
          },
          content: {
            systemPrompt: '系统提示',
            userMessage: '用户消息',
            received: '响应内容',
            truncated: '已截断',
            copy: '复制',
            copied: '已复制',
            empty: '无内容',
          },
          error: '错误信息',
          errorType: '错误类型',
          errorMessage: '错误描述',
        },
      },
      sync: {
        drawerSubtitle: '确认术语变更对各资源的影响，选择同步范围后执行',
        // 抽屉标题
        titleImpact: '术语同步更新',
        titleExecuting: '术语同步更新 — 执行中',
        titleCompleted: '术语同步更新 — 已完成',
        titleCancelled: '术语同步更新 — 已取消',
        titleError: '术语同步更新 — 失败',

        // 步骤指示器
        stepImpact: '影响分析',
        stepExecute: '同步执行',
        stepResult: '结果摘要',

        // 步骤 1: 影响分析
        targetChanged: '术语译文已更新，以下是新旧译文对比：',
        targetChangedWithSource: '术语「{source}」的译文已更新，以下是新旧译文对比：',
        noImpact: '当前已翻译段落中未发现需要同步的内容',
        impactSummary: '发现 {count} 个已翻译段落包含旧译文，分布在 {resourceCount} 个资源文件中',
        impactLoadFailed: '影响分析加载失败，请重试',
        resourcePath: '资源文件',
        affectedCount: '受影响段落',
        skip: '跳过',
        skipAndNext: '跳过此项',
        continueNext: '继续下一项',
        queueProgress: '第 {current} / {total} 项',
        syncSelected: '限定范围同步（{count}）',
        syncAll: '全部同步',
        retry: '重试',

        // 步骤 2: 执行进度
        pending: '等待中',
        executingWithSource: '正在同步术语「{source}」的译文…',
        progress: '已处理 {processed} / {total} 段落',
        cancel: '取消同步',

        // 步骤 3: 结果摘要
        completed: '术语译文同步完成',
        updated: '已更新',
        skipped: '已跳过',
        updatedCount: '已更新',
        skippedCount: '已跳过',
        reviewHint: '已更新的段落状态已设为「待审核」，请在段落列表中检查并确认',

        // 取消状态
        cancelled: '术语译文同步已取消',
        processed: '已处理',
        unprocessed: '未处理',
        cancelledHint: '已更新的段落已保留在数据库中，未处理的段落仍保留旧译文',

        // 错误状态
        unknownError: '同步过程中发生未知错误',
        networkError: '网络连接异常，轮询已停止。请检查网络后重试',
        taskFailed: '同步任务执行失败',
        executeFailed: '同步任务提交失败',

        // Store 方法错误
        impactFailed: '影响分析请求失败',
      },
    },
    messages: {
      uploadSuccess: '资源已上传',
      uploadPartialSuccess:
        '上传部分完成：创建 {created}，增量更新 {incrementallyUpdated}，覆盖 {replaced}，跳过 {skipped}，失败 {failed}',
      uploadFailed: '资源上传失败',
      replaceSuccess: '资源已替换',
      replaceFailed: '资源替换失败',
      deleteResourceSuccess: '资源已删除',
      deleteResourceFailed: '资源删除失败',
      downloadFailed: '下载失败',
      incrementalUpdateSuccess: '增量更新完成',
      incrementalUpdateFailed: '增量更新失败',
      segmentSaved: '段落已保存',
      segmentSaveFailed: '段落保存失败',
      jobCreated: '任务已创建',
      jobCreateFailed: '任务创建失败',
      jobDetailFailed: '任务详情加载失败',
      jobCancelled: '任务已取消',
      jobCancelFailed: '任务取消失败',
      jobRetried: '任务已重试',
      jobRetryFailed: '任务重试失败',
      jobPaused: '任务已暂停',
      jobPauseFailed: '暂停任务失败',
      jobResumed: '任务已恢复',
      jobResumeFailed: '恢复任务失败',
      selectReadyResource: '请选择至少一个资源',
      selectResourceFirst: '请先选择资源文件',
      copyFailed: '复制失败，剪贴板不可用',
    },
  },
  backends: {
    title: 'AI 后端',
    subtitle: '接入并管理 OpenAI、Anthropic、Google Gemini 等模型服务',
    filters: {
      all: '全部',
      searchPlaceholder: '搜索后端名称或模型',
      reset: '重置筛选',
    },
    types: {
      openai: 'OpenAI',
      anthropic: 'Anthropic',
      google: 'Gemini',
    },
    card: {
      model: '模型',
      thinking: '思考强度',
      baseUrl: 'Base URL',
      rateLimit: '限速',
      rateLimitValue: '{n} 次/分',
    },
    empty: {
      default: '暂无 AI 后端，添加一个开始使用翻译服务',
      filtered: '没有找到符合条件的后端',
    },
    actions: {
      copy: '复制',
    },
    create: {
      title: '添加 AI 后端',
      description: '选择后端类型并填写配置信息。API Key 等敏感信息将安全存储',
    },
    edit: {
      title: '编辑 AI 后端',
      description: '更新后端配置信息。修改后立即生效',
    },
    copy: {
      name: '{name} 副本',
    },
    delete: {
      confirm: '确定要删除后端「{name}」吗？此操作无法撤销。',
    },
    form: {
      name: '后端名称',
      namePlaceholder: '例如：My OpenAI',
      type: '后端类型',
      typePlaceholder: '选择后端类型',
      options: '高级配置',
      apiKey: 'API Key',
      apiKeyPlaceholder: 'sk-…',
      apiKeyRequired: 'API Key 为必填项',
      baseUrl: 'Base URL（可选）',
      baseUrlPlaceholder: '留空使用官方地址',
      model: '模型',
      modelPlaceholder: '例如：gpt-4o-mini，或探测后选择',
      fetchModels: '探测模型',
      fetchModelsHint: '使用当前 API Key 与 Base URL 向服务商拉取可用模型，凭据不会落库',
      fetchModelsNeedCredentials: '请先选择后端类型并填写 API Key',
      fetchModelsSuccess: '已加载 {count} 个可用模型',
      fetchModelsEmpty: '未获取到可用模型，可手动输入模型 ID',
      fetchModelsAuthFailed: '认证失败：API Key 无效或未授权',
      fetchModelsForbidden: '没有权限访问该服务的模型列表',
      fetchModelsNotFound: '模型列表接口不可用，请检查 Base URL',
      fetchModelsNetworkFailed: '无法连接服务，请检查 Base URL 与网络',
      temperature: 'Temperature',
      topP: 'Top P',
      useApiDefault: '使用 API 默认值',
      thinkingLevel: '思考强度',
      thinkingLevelPlaceholder: '选择思考强度',
      thinkingLevelHint:
        'off 表示不参与思考控制，沿用模型/网关默认；low/medium/high 仅对支持推理的模型生效',
      thinkingLevelAnthropicHint:
        'Anthropic 开启思考后会忽略 Temperature / Top P，且思考预算与输出共用 Max Tokens',
      thinkingLevels: {
        off: '关闭（默认）',
        low: '低',
        medium: '中',
        high: '高',
      },
      samplingIgnoredByThinking: '开启思考后由服务端忽略',
      maxTokens: 'Max Tokens',
      maxTokensPlaceholder: '0 表示不限制',
      maxTokensThinkingHint:
        '思考与最终输出共用同一 Max Tokens 池（约 low 25% / medium 50% / high 75% 给思考），建议适当提高上限',
      timeout: '超时时间（秒）',
      timeoutPlaceholder: '例如：60',
      timeoutUnlimited: '不限制',
      responseFormat: '响应格式',
      responseFormatOptions: {
        jsonSchema: 'JSON Schema（结构化输出）',
        jsonObject: 'JSON 对象',
        text: '纯文本',
        none: '不指定',
      },
      enablePromptCache: '启用 Prompt Cache',
      stream: '流式请求',
      streamHint: '以 stream 方式请求并在内部累积完整响应，适用于仅支持 stream:true 的兼容网关',
      rateLimitPerMinute: '每分钟请求限制',
      rateLimitPerMinutePlaceholder: '0 表示不限速',
      rateLimitPerMinuteHint: '限制每分钟发送到此后端的最大请求数，0 表示不限速',
    },
    validation: {
      nameRequired: '请输入后端名称',
      typeRequired: '请选择后端类型',
      apiKeyRequired: '请输入 API Key',
      modelRequired: '请输入模型名称',
    },
    messages: {
      createSuccess: '后端已创建',
      createFailed: '后端创建失败，请稍后重试',
      updateSuccess: '后端已更新',
      updateFailed: '后端更新失败，请稍后重试',
      deleteSuccess: '后端已删除',
      deleteFailed: '后端删除失败，请稍后重试',
    },
  },
  promptTemplates: {
    title: '翻译提示词',
    subtitle: '定义 AI 翻译的行为指令与输出规则，翻译任务执行时套用',
    actions: {
      create: '新建模板',
      createFirst: '创建第一个模板',
      submitCreate: '创建模板',
      submitUpdate: '保存',
      createTitle: '新建翻译提示词',
      editTitle: '编辑翻译提示词',
      viewTitle: '查看翻译提示词',
    },
    filters: {
      searchPlaceholder: '搜索模板名称或描述',
      all: '全部',
      reset: '重置筛选',
    },
    scopes: {
      system: '系统',
      user: '用户',
    },
    card: {
      updatedAt: '更新于',
      noDescription: '暂无描述',
      noContent: '暂无提示词内容',
    },
    form: {
      name: '模板名称',
      namePlaceholder: '例如：技术文档翻译提示词',
      description: '模板描述',
      descriptionPlaceholder: '简要说明模板的用途和适用场景',
      content: '系统提示词',
      contentPlaceholder: '输入提示词内容',
      insertBuiltinVar: '插入变量：',
      createHint: '名称与描述便于识别，提示词将在翻译任务中作为系统指令生效',
    },
    validation: {
      nameRequired: '请输入模板名称',
    },
    messages: {
      createSuccess: '翻译提示词已创建',
      updateSuccess: '翻译提示词已更新',
      deleteSuccess: '翻译提示词已删除',
      systemDeleteForbidden: '系统模板不可删除',
    },
    empty: {
      default: '还没有翻译提示词，创建一个来定义翻译行为',
      filtered: '没有找到符合条件的模板',
    },
    delete: {
      confirm: '确定要删除翻译提示词「{name}」吗？此操作无法撤销。',
    },
    variables: {
      system: {
        SourceLang: '源语言',
        TargetLang: '目标语言',
        Protocol: '协议（text / json_loose / json_strict）',
        SourceContent: '源内容',
        TargetContent: '目标内容',
        GlossaryTerms: '术语表',
        InlineBootstrap: '是否内联术语抽取',
        MaxBootstrapTerms: '内联术语上限',
        RubyMode: '注音模式（json / section / inline）',
        FileFormat: '文件格式',
        FileName: '文件名',
        OriginalText: '原始文本',
        TranslatedText: '已翻译文本',
      },
      bootstrap: {
        SourceLang: '源语言',
        TargetLang: '目标语言',
        Protocol: '协议（text / json_loose / json_strict）',
        MaxTerms: '最大术语数',
      },
      prune: {
        SourceLang: '源语言',
        TargetLang: '目标语言',
        Protocol: '协议（text / json_loose / json_strict）',
        Entries: '完整术语条目集合',
      },
    },
  },
  bootstrapPromptTemplates: {
    title: '术语抽取提示词',
    subtitle: '定义从源文本中抽取翻译术语的指令，供术语库构建使用',
    actions: {
      create: '新建模板',
      createFirst: '创建第一个模板',
      submitCreate: '创建模板',
      submitUpdate: '保存',
      createTitle: '新建术语抽取提示词',
      editTitle: '编辑术语抽取提示词',
      viewTitle: '查看术语抽取提示词',
    },
    filters: {
      searchPlaceholder: '搜索模板名称或描述',
      all: '全部',
      reset: '重置筛选',
    },
    scopes: {
      system: '系统',
      user: '用户',
    },
    card: {
      updatedAt: '更新于',
      noDescription: '暂无描述',
      noContent: '暂无提示词内容',
    },
    form: {
      name: '模板名称',
      namePlaceholder: '例如：通用术语抽取提示词',
      description: '模板描述',
      descriptionPlaceholder: '简要说明模板的用途和适用场景',
      content: '术语抽取提示词',
      contentPlaceholder: '输入术语抽取提示词内容',
      insertBuiltinVar: '插入变量：',
      createHint: '名称与描述便于识别，提示词将在术语抽取时生效',
    },
    validation: {
      nameRequired: '请输入模板名称',
    },
    messages: {
      createSuccess: '术语抽取提示词已创建',
      updateSuccess: '术语抽取提示词已更新',
      deleteSuccess: '术语抽取提示词已删除',
      systemDeleteForbidden: '系统模板不可删除',
    },
    empty: {
      default: '还没有术语抽取提示词，创建一个来定义抽取规则',
      filtered: '没有找到符合条件的模板',
    },
    delete: {
      confirm: '确定要删除术语抽取提示词「{name}」吗？此操作无法撤销。',
    },
  },
  prunePromptTemplates: {
    title: '术语精简提示词',
    subtitle: '定义术语表精简的分析指令，控制术语的删除、合并与译文优化',
    actions: {
      create: '新建模板',
      createFirst: '创建第一个模板',
      submitCreate: '创建模板',
      submitUpdate: '保存',
      createTitle: '新建术语精简提示词',
      editTitle: '编辑术语精简提示词',
      viewTitle: '查看术语精简提示词',
    },
    filters: {
      searchPlaceholder: '搜索模板名称或描述',
      all: '全部',
      reset: '重置筛选',
    },
    scopes: {
      system: '系统',
      user: '用户',
    },
    card: {
      updatedAt: '更新于',
      noDescription: '暂无描述',
      noContent: '暂无提示词内容',
    },
    form: {
      name: '模板名称',
      namePlaceholder: '例如：通用术语精简提示词',
      description: '模板描述',
      descriptionPlaceholder: '简要说明模板的用途和适用场景',
      content: '精简提示词',
      contentPlaceholder: '输入用于分析术语表的提示词内容',
      insertBuiltinVar: '插入变量：',
      createHint: '名称与描述便于识别，提示词将在术语表精简时生效',
    },
    validation: {
      nameRequired: '请输入模板名称',
    },
    messages: {
      createSuccess: '术语精简提示词已创建',
      updateSuccess: '术语精简提示词已更新',
      deleteSuccess: '术语精简提示词已删除',
      systemDeleteForbidden: '系统模板不可删除',
    },
    empty: {
      default: '暂无自定义精简提示词，精简时将使用内置默认模板',
      filtered: '没有找到符合条件的模板',
    },
    delete: {
      confirm: '确定要删除术语精简提示词「{name}」吗？此操作无法撤销。',
    },
  },
  executionProfiles: {
    title: '执行策略',
    subtitle: '定义翻译任务执行时的保护、注音、修复、后处理与质检规则，供执行计划引用',
    actions: {
      create: '新建策略',
      createFirst: '创建第一个策略',
      createTitle: '新建执行策略',
      editTitle: '编辑执行策略',
      viewTitle: '查看执行策略',
      submitCreate: '创建策略',
      submitUpdate: '保存',
    },
    filters: {
      searchPlaceholder: '搜索策略名称或描述',
      all: '全部',
      reset: '重置筛选',
    },
    scopes: {
      system: '系统',
      user: '用户',
    },
    card: {
      updatedAt: '更新于',
      noDescription: '暂无描述',
      noFeatures: '未启用任何处理能力',
    },
    feature: {
      protect: '保护',
      ruby: '注音',
      repair: '修复',
      postprocess: '后处理',
      glossary: '术语表',
      context: '上下文',
      qa: '质检',
    },
    form: {
      name: '策略名称',
      namePlaceholder: '例如：技术文档执行策略',
      description: '策略描述',
      descriptionPlaceholder: '简要说明策略的用途和适用场景',
      createHint: '名称与描述便于识别，下方按需开启各处理能力',
    },
    validation: {
      nameRequired: '请输入策略名称',
    },
    messages: {
      createSuccess: '执行策略创建成功',
      createFailed: '执行策略创建失败',
      updateSuccess: '执行策略已更新',
      updateFailed: '执行策略更新失败',
      deleteSuccess: '执行策略已删除',
      deleteFailed: '执行策略删除失败',
      systemDeleteForbidden: '系统策略不可删除',
    },
    empty: {
      default: '暂无执行策略，创建一个开始定义执行流程',
      filtered: '没有找到符合条件的策略',
    },
    delete: {
      confirm: '确定要删除执行策略「{name}」吗？此操作无法撤销。',
    },
  },
  profileConfigEditor: {
    protect: {
      title: '内容保护',
      description: '防止译文破坏代码块、链接、占位符等原文结构',
      enabled: '启用保护',
      rules: '保护规则',
      ruleOptions: {
        code: '代码块',
        link: '链接',
        placeholder: '占位符',
        xml: 'XML 标签',
      },
    },
    ruby: {
      title: '注音处理',
      description: '为 CJK 文本生成注音，并按分类决定保留范围',
      enabled: '启用处理',
      preserveKinds: '保留注音分类',
      preserveKindsPhonetic: '音注',
      preserveKindsSemantic: '义训',
      preserveKindsCreative: '创意',
    },
    postprocess: {
      title: '后处理',
      description: '译文落库前的确定性清理',
      enabled: '启用后处理',
      trimSpaces: '去除多余空格',
    },
    repair: {
      title: '响应修复',
      description: '自动修复 LLM 响应中的常见格式问题',
      enabled: '启用响应修复',
      jsonStructural: 'JSON 结构修复',
      schemaAliases: 'Schema 别名映射',
      placeholderNormalize: '占位符归一化',
      promptUpgrade: 'Prompt 升级重试',
    },
    glossary: {
      title: '术语表',
      description: '翻译前从源文内联抽取术语并注入提示词',
      bootstrapEnabled: '启用内联自举',
      bootstrapMaxTerms: '每千字符最大术语数',
      bootstrapMinSourceLen: '最短源术语长度',
      bootstrapConflictStrategy: '冲突策略',
      conflictStrategyOptions: {
        off: '关闭',
        rewriteLocal: '以术语表为准',
      },
    },
    context: {
      title: '上下文窗口',
      description: '为每段译文附带前后文段落，提升连贯性',
      enabled: '启用上下文窗口',
      before: '前文段落数',
      after: '后文段落数',
      maxChars: '段落字符上限',
      maxCharsHint: '每个上下文段落的字符数上限（按 rune 计，超限截断并补省略号），0 表示不限制',
    },
    qa: {
      title: '质量检测',
      description: '对译文执行确定性规则检查，未通过可自动拒绝',
      enabled: '启用质量检测',
      autoReject: '错误自动拒绝',
      lengthMethod: '长度计算方式',
      lengthMethodCharWeight: '字符加权（CJK×2）',
      lengthMethodWordCount: '字（词）计数',
      lengthRatioMin: '最小长度比',
      lengthRatioMax: '最大长度比',
      lengthRatioHint: '比值 = 译文长度 / 原文长度，0 表示不检测',
      lengthRatioMinMaxError: '最小长度比不能大于最大长度比',
      checksTitle: '确定性检查项',
      checksAll: '启用全部检查项',
      checksAllHint: '省略具体检查项即表示全部开启',
      checksCustom: '自定义选择',
      checksHint: '勾选要启用的确定性检查项',
      checksEmpty: '至少选择一项，或切回「启用全部」',
      checks: {
        untranslated: '未翻译',
        length_ratio: '长度异常',
        duplicate: '重复译文',
        source_residual: '源文残留',
        punctuation_pairing: '标点配对',
        punctuation_missing: '标点缺失',
        punctuation_surplus: '标点多余',
        punctuation_wrap_loss: '引号包裹缺失',
        whitespace_irregular: '空白异常',
        repeated_space: '重复空格',
        width_mix: '全半角混用',
        script_mismatch: '文字系统混用',
        number_mismatch: '数字不一致',
        url_email_mismatch: '链接邮箱不一致',
        subtitle_line_count: '字幕行数',
        forbidden_term: '禁译词',
        term_inconsistency: '术语不一致',
        leftover_placeholder: '占位符残留',
        xml_tag_mismatch: 'XML 标签不一致',
        duplicate_source_divergence: '同源译文偏差',
      },
    },
  },
  executionPlanTemplates: {
    title: '执行计划',
    subtitle: '编排翻译任务的轮次流程，安排每轮的模式、AI 后端与并发参数',
    actions: {
      create: '新建计划',
      createFirst: '创建第一个计划',
      createTitle: '新建执行计划',
      editTitle: '编辑执行计划',
      viewTitle: '查看执行计划',
      submitCreate: '创建计划',
      submitUpdate: '保存',
    },
    filters: {
      searchPlaceholder: '搜索计划名称或描述',
      all: '全部',
      reset: '重置筛选',
    },
    scopes: {
      system: '系统',
      user: '用户',
    },
    card: {
      updatedAt: '更新于',
      noDescription: '暂无描述',
      profile: '策略',
    },
    form: {
      name: '计划名称',
      namePlaceholder: '例如：多轮精翻计划',
      description: '计划描述',
      descriptionPlaceholder: '简要说明计划的执行策略',
      createHint: '命名计划并选择执行策略，再按顺序编排轮次',
      profile: '执行策略',
      profilePlaceholder: '选择计划引用的执行策略',
      profileHint:
        '计划级策略引用：为全部改写型轮次与引擎级行为提供保护/注音/后处理/修复/术语表/上下文/质检预设，任务创建时冻结进执行快照',
      rounds: '执行轮次',
    },
    validation: {
      nameRequired: '请输入计划名称',
      profileRequired: '请选择执行策略',
      roundBackendRequired: '轮次 {n}：请选择 AI 后端',
      roundPromptRequired: '轮次 {n}：请选择提示词模板',
      roundBatchSizeRequired: '轮次 {n}：请设置批次大小',
      roundBatchConfigRequired: '轮次 {n}：批次大小和每批字词数至少填一项',
      roundConcurrencyRequired: '轮次 {n}：请设置并发数',
      roundSemanticQAIssueCodesRequired: '轮次 {n}：按问题代码筛选时至少选择一个问题代码',
      roundReviseIssueCodesRequired: '轮次 {n}：修订轮按问题代码筛选时至少选择一个问题代码',
      roundFallbackShrinkRequired: '轮次 {n}：缩放因子需在 0 到 1 之间（不含 0）',
      roundCorrectRulesRequired: '轮次 {n}：本地改写至少启用一条规则',
    },
    messages: {
      createSuccess: '执行计划已创建',
      createFailed: '执行计划创建失败',
      updateSuccess: '执行计划已更新',
      updateFailed: '执行计划更新失败',
      deleteSuccess: '执行计划已删除',
      deleteFailed: '执行计划删除失败',
      systemDeleteForbidden: '系统计划不可删除',
    },
    empty: {
      default: '暂无执行计划，创建一个开始定义翻译流程',
      filtered: '没有找到符合条件的计划',
    },
    delete: {
      confirm: '确定要删除执行计划「{name}」吗？此操作无法撤销。',
    },
  },
  executionPlanEditor: {
    rubyRetry: {
      title: '注音对齐重试',
      description: '注音对齐失败时自动重试，可指定独立后端',
      enabled: '启用注音对齐重试',
      backend: '注音对齐后端',
      backendPlaceholder: '留空则使用翻译主后端',
      maxAttempts: '重试轮数',
      maxAttemptsHint: '仅启用时生效，最少 1 轮',
    },
    round: {
      mode: '轮次模式',
      modeTranslate: '翻译',
      modeExtract: '术语抽取',
      modeAdjudicate: '质量裁决',
      modeSemanticQA: '语义质检',
      modeRevise: 'LLM 修订',
      modeCorrect: '本地改写',
      backend: 'AI 后端',
      backendPlaceholder: '选择后端',
      concurrency: '并发数',
      promptTemplate: '提示词模板',
      promptTemplatePlaceholder: '选择提示词模板',
      batchSize: '批次大小',
      batchSizeHint: '每批处理的待译段落数上限（不计上下文段），0 表示不限制',
      maxWordsPerBatch: '每批字词数',
      maxWordsPerBatchHint: '每批处理的字词数上限（计入上下文段），0 表示不限制',
      fallbackShrink: '缩放因子',
      fallbackShrinkPlaceholder: '取值范围 0 < x ≤ 1',
      fallbackShrinkHint: '1 = 不缩放，小于 1 表示每池逐级缩小',
      advancedConfig: '高级配置',
      retryMaxAttempts: '最大重试次数',
      retryBackoffMs: '重试退避间隔（毫秒）',
      retryJitter: '启用抖动',
      segmentFilter: '段落过滤',
      segmentFilterHint: '决定处理哪些翻译状态的段落，默认仅待翻译',
      segmentFilterPendingOnly: '仅待翻译',
      segmentFilterSkipApproved: '跳过已批准',
      segmentFilterAll: '全部段落',
      extractTemplate: '术语抽取模板',
      extractTemplatePlaceholder: '选择术语抽取模板',
      noBatch: '不分批，全部一次发送',
      extractBatchSize: '段落数上限',
      extractBatchSizeHint: '0 表示不限制，与字词数上限至少填一项',
      extractMaxWordsPerBatch: '字词数上限',
      extractMaxWordsPerBatchHint: '0 表示不限制，与段落数上限至少填一项',
      extractMaxTerms: '每千字术语抽取系数',
      extractMinSourceLen: '术语最短源文长度',
      adjudicatePromptHint: '裁决系统提示词已内置，无需选择提示词模板',
      adjudicateBatchSize: '段落数上限',
      adjudicateBatchSizeHint: '0 表示不限制，与字词数上限至少填一项',
      adjudicateMaxWordsPerBatch: '字词数上限',
      adjudicateMaxWordsPerBatchHint: '0 表示不限制，与段落数上限至少填一项',
      adjudicateCodes: '可裁决问题',
      adjudicateCodesPlaceholder: '默认：源文残留 + 标点多余',
      adjudicateCodesHint: '重复译文为硬规则，不可裁决。空则默认源文残留、标点多余',
      adjudicateCodeSourceResidual: '源文残留',
      adjudicateCodeLengthRatio: '长度异常',
      adjudicateCodePunctuationSurplus: '标点多余',
      adjudicateCodeUntranslated: '未翻译',
      semanticQAPromptHint:
        '语义质检系统提示词已内置，无需选择提示词模板；产出的警告级语义问题直接进入人工审核',
      semanticQASegmentScope: '段落扫描范围',
      semanticQASegmentScopePlaceholder: '选择扫描范围',
      semanticQASegmentScopeHint:
        '控制语义质检覆盖哪些段落。缩小范围可降低成本，未扫描段落保留原有语义问题',
      semanticQASegmentScopeAll: '全部译文段落',
      semanticQASegmentScopeWithIssues: '仅有问题的段落',
      semanticQASegmentScopeWithIssueCodes: '按问题代码筛选',
      semanticQAIssueCodes: '问题代码筛选',
      semanticQAIssueCodesPlaceholder: '选择要匹配的问题代码',
      semanticQAIssueCodesHint: '仅扫描包含所选问题代码的段落，适合成本敏感的高价值子集',
      semanticQABatchSize: '段落数上限',
      semanticQABatchSizeHint: '0 表示不限制，与字词数上限至少填一项',
      semanticQAMaxWordsPerBatch: '字词数上限',
      semanticQAMaxWordsPerBatchHint: '0 表示不限制，与段落数上限至少填一项',
      revisePromptHint:
        '修订系统提示词已内置，无需选择提示词模板；LLM 对现有译文做最小改动定点修订，仅修复未决的语义问题，写回时不改变段落状态',
      reviseSegmentScope: '段落扫描范围',
      reviseSegmentScopePlaceholder: '选择扫描范围',
      reviseSegmentScopeHint:
        '仅处理已翻译/已编辑且译文非空的段落；默认修订所有存在未决语义问题的段落',
      reviseSegmentScopeWithIssues: '存在未决语义问题的段落',
      reviseSegmentScopeWithIssueCodes: '按问题代码筛选',
      reviseIssueCodes: '问题代码筛选',
      reviseIssueCodesPlaceholder: '选择要修复的问题代码',
      reviseIssueCodesHint: '仅修复包含所选问题代码的未决语义问题（语义白名单子集）',
      reviseBatchSize: '段落数上限',
      reviseBatchSizeHint: '0 表示不限制，与字词数上限至少填一项',
      reviseMaxWordsPerBatch: '字词数上限',
      reviseMaxWordsPerBatchHint: '0 表示不限制，与段落数上限至少填一项',
      correctPromptHint:
        '本地改写纯本地机械修复，不调用 LLM、不分批、无重试；规则按顺序执行，首个生效即停',
      correctRules: '改写规则',
      correctRulesHint: '按顺序执行，至少启用一条规则',
      correctRulePunctuationMissingWrap: '补全缺失引号包裹',
      correctRulePunctuationMissingWrapHint:
        '机械修复 punctuation_missing 报出的译文丢失源文引号包裹（单 span 安全子集）',
      correctRulePunctuationWrapLossWrap: '补全丢失的外层引号包裹',
      correctRulePunctuationWrapLossWrapHint:
        '机械修复 punctuation_wrap_loss 报出的译文丢失源文外层引号包裹（单 span 安全子集）',
      correctRuleWidthMixNormalize: '规范全半角混用',
      correctRuleWidthMixNormalizeHint:
        '修复 width_mix 报出的全半角混用：CJK 译文把 9 个零歧义半角标点（! ? , ; : ( ) [ ]）转全角（数字双侧的 , : 与数字前缀的 !? 豁免，保留 1,000/12:30/5! 原样）；拉丁译文把全角字符（FF01-FF5E，含全角字母数字）转回半角',
    },
    actions: {
      addRound: '添加轮次',
      moveUp: '上移',
      moveDown: '下移',
      removeRound: '删除轮次',
    },
  },
  about: {
    title: '关于 LinguaFlow',
    description:
      'LinguaFlow 是一个 AI 驱动的多语言翻译工作台，支持将文档、字幕、电子书等内容批量翻译成多种语言；内置术语管理、翻译审核与进度追踪，让翻译工作更高效、更准确',
    supportedFormats: '支持的文件格式',
    coreFeatures: '核心功能',
    featureProject: '项目管理',
    featureProjectDesc: '以项目为单位组织翻译工作，支持数十种语言互译，项目级别术语表管理',
    featureTranslation: '智能翻译',
    featureTranslationDesc:
      '集成 OpenAI、Anthropic、Google Gemini，自动检测源语言，上下文感知，特殊内容自动保护',
    featureBatch: '批量处理',
    featureBatchDesc: '拖拽上传文件或文件夹，增量更新保留已有译文，并发翻译，实时进度追踪',
    featureEpub: 'EPUB 电子书',
    featureEpubDesc: '按章节浏览和翻译，章节级进度追踪，HTML 预览，保留原始格式',
    featureGlossary: '术语管理',
    featureGlossaryDesc: 'CSV 导入导出，自动术语提取，术语修改后自动同步已翻译段落',
    featureReview: '翻译审核',
    featureReviewDesc: '按状态筛选，行内编辑译文，批量批准/拒绝，添加备注',
    featureConfig: '灵活配置',
    featureConfigDesc: '自定义提示词、执行策略、执行计划，定义多轮执行流程',
    techStack: '技术架构',
    techFrontend: '前端',
    techBackend: '后端',
    techDatabase: '数据库',
    techApi: 'API',
    license: '许可证',
    licenseDesc: '本项目采用双许可模式：开源使用遵循 AGPL v3 协议，商业许可即将推出',
    licenseAgpl:
      '开源用户：修改后的代码需开源。商业用户：如需闭源使用或私有部署，请等待商业许可上线',
  },
  changelog: {
    title: '更新日志',
    description: '查看 LinguaFlow 的版本更新记录，了解新功能、改进和修复',
    fetchError: '获取更新日志失败',
    retry: '重试',
    empty: '暂无更新记录',
    viewOnGithub: '在 GitHub 查看',
    noDescription: '该版本暂无详细说明',
  },
  admin: {
    title: '管理后台',
    subtitle: '管理系统用户、查看全局统计、审计日志和系统配置',
    dashboard: {
      title: '系统概览',
      description: '查看系统全局统计数据和关键指标',
      stats: {
        totalUsers: '总用户数',
        activeUsers: '活跃用户',
        totalProjects: '总项目数',
        totalOrganizations: '总组织数',
        totalJobs: '总任务数',
        totalResources: '总资源数',
      },
    },
    users: {
      title: '用户管理',
      description: '管理系统用户账号，创建、编辑、停用用户',
      actions: {
        create: '创建用户',
        edit: '编辑',
        disable: '停用',
        resetPassword: '重置密码',
      },
      stats: {
        total: '全部用户',
        active: '正常用户',
        admins: '管理员',
      },
      filters: {
        searchPlaceholder: '搜索用户名、邮箱或显示名',
        allRoles: '全部角色',
        allStatuses: '全部状态',
        active: '正常',
        inactive: '已停用',
        reset: '重置筛选',
      },
      roles: {
        admin: '管理员',
        user: '普通用户',
      },
      columns: {
        username: '用户',
        email: '邮箱',
        displayName: '显示名',
        role: '角色',
        status: '状态',
        actions: '操作',
      },
      create: {
        title: '创建用户',
        description: '填写用户信息创建新账号',
      },
      edit: {
        title: '编辑用户',
        description: '修改用户信息和权限',
      },
      resetPassword: {
        title: '重置密码',
        description: '为用户设置新密码',
        newPassword: '新密码',
        newPasswordPlaceholder: '至少 8 位',
        confirm: '确认重置',
      },
      disable: {
        confirm: '确定要停用用户「{name}」吗？停用后该用户将无法登录。',
      },
      form: {
        username: '用户名',
        usernamePlaceholder: '3-32 位，字母 / 数字 / 下划线',
        email: '邮箱',
        emailPlaceholder: "user{'@'}example.com",
        displayName: '显示名',
        displayNamePlaceholder: '留空则使用用户名',
        password: '密码',
        passwordPlaceholder: '至少 8 位',
        role: '角色',
        rolePlaceholder: '选择用户角色',
      },
      validation: {
        usernameRequired: '请输入用户名',
        usernameLength: '用户名长度需在 3-32 之间',
        emailRequired: '请输入邮箱',
        emailInvalid: '请输入合法的邮箱地址',
        passwordRequired: '请输入密码',
        passwordMinLength: '密码至少 8 位',
        roleRequired: '请选择角色',
      },
      messages: {
        createSuccess: '用户已创建',
        createFailed: '用户创建失败',
        updateSuccess: '用户已更新',
        updateFailed: '用户更新失败',
        disableSuccess: '用户已停用',
        disableFailed: '用户停用失败',
        resetPasswordSuccess: '密码已重置',
        resetPasswordFailed: '密码重置失败',
      },
      empty: {
        default: '暂无用户',
        filtered: '没有找到符合条件的用户',
      },
    },
    auditLogs: {
      title: '审计日志',
      description: '查看系统操作审计记录，追踪关键操作历史',
      columns: {
        time: '时间',
        actor: '操作者',
        action: '操作',
        resource: '资源',
        details: '详情',
      },
      actions: {
        jobCreate: '创建任务',
        jobCancel: '取消任务',
        jobRetry: '重试任务',
        segmentUpdate: '编辑段落',
        segmentApprove: '批准',
        segmentReject: '拒绝',
        segmentBatchReview: '批量审核',
        segmentApproveAll: '全部批准',
        segmentRetranslateRejected: '重译已拒绝段落',
        glossarySync: '术语同步',
        quickTranslate: '即时翻译',
        segmentPreviewApply: '应用预览翻译',
      },
      resourceTypes: {
        job: '任务',
        project: '项目',
        resource: '资源',
        segment: '段落',
        glossary_entry: '术语条目',
        quick_translate: '即时翻译',
      },
      empty: '暂无审计日志',
      refresh: '刷新',
    },
    settings: {
      title: '系统设置',
      description: '查看和修改系统全局配置参数',
      actions: {
        save: '保存设置',
        refresh: '刷新',
        addSetting: '添加配置项',
      },
      form: {
        key: '配置键',
        keyPlaceholder: '输入配置键名',
        value: '配置值',
        valuePlaceholder: '输入配置值',
      },
      messages: {
        saveSuccess: '系统设置已保存',
        saveFailed: '保存系统设置失败',
        loadFailed: '加载系统设置失败',
      },
      keyCount: '{count} 个配置项',
      empty: '暂无系统配置项',
      deleteConfirm: '确定要删除该配置项吗？',
    },
  },
  notFound: {
    code: '404',
    title: '页面不存在',
    description: '你访问的页面不存在或已被移动',
    backHome: '返回工作台',
    backPrev: '返回上一页',
  },
  globalJobTracker: {
    title: '任务追踪',
    activeCount: '{count} 个活跃任务',
    noTrackedJobs: '暂无追踪任务',
    clearCompleted: '清除已完成',
    dismiss: '移除',
    viewDetail: '查看详情',
    goToProject: '前往项目',
    close: '关闭',
    progress: '进度',
    segments: '{completed}/{total} 段',
    project: '所属项目',
    detailTitle: '任务详情 #{id}',
    detailFallbackTitle: '任务详情',
    status: {
      pending: '等待中',
      running: '运行中',
      completed: '已完成',
      failed: '失败',
      cancelled: '已取消',
    },
  },
  api: {
    errors: {
      requestNotSent: '请求未送达，请检查网络或服务器地址',
      serverReturned: '服务器返回 {status}',
      pingFailed: '连接服务器失败',
      fetchModeFailed: '获取运行模式失败',
      loginFailed: '登录失败',
      registerFailed: '注册失败',
      refreshSessionFailed: '刷新会话失败',
      fetchCurrentUserFailed: '获取当前用户失败',
      fetchStatsFailed: '获取用量统计失败',
      fetchActivityFailed: '获取活动日志失败',
      fetchProjectsFailed: '获取项目列表失败',
      fetchProjectFailed: '获取项目详情失败',
      createProjectFailed: '创建项目失败',
      updateProjectFailed: '更新项目失败',
      deleteProjectFailed: '删除项目失败',
      fetchResourceTreeFailed: '获取资源目录树失败',
      fetchResourcesFailed: '获取资源列表失败',
      uploadResourcesFailed: '上传资源失败',
      precheckResourcesFailed: '上传预检失败',
      replaceResourceFailed: '替换资源失败',
      incrementalUpdateFailed: '增量更新失败',
      uploadResourceConflict: '上传的文件与已有资源冲突',
      deleteResourceFailed: '删除资源失败',
      downloadResourceFailed: '下载资源失败',
      fetchSegmentsFailed: '获取段落列表失败',
      updateSegmentFailed: '更新段落失败',
      batchReviewSegmentsFailed: '批量审核段落失败',
      approveAllSegmentsFailed: '全部批准段落失败',
      setIssueDispositionFailed: '裁决质量问题失败',
      fetchJobsFailed: '获取任务失败',
      fetchJobFailed: '获取任务详情失败',
      createJobFailed: '创建任务失败',
      cancelJobFailed: '取消任务失败',
      retryJobFailed: '重试任务失败',
      pauseJobFailed: '暂停任务失败',
      resumeJobFailed: '恢复任务失败',
      listJobEventsFailed: '获取任务事件失败',
      downloadJobFailed: '下载结果失败',
      downloadResourceResultFailed: '下载结果失败',
      downloadResourceResultEmpty: '该资源暂无结果',
      downloadTranslatedFailed: '下载译文失败',
      loadStatsFailed: '加载统计失败',
      loadActivityFailed: '加载活动失败',
      loadProjectsFailed: '加载项目失败',
      fetchBackendsFailed: '获取后端列表失败',
      listBackendModelsFailed: '探测可用模型失败',
      createBackendFailed: '创建后端失败',
      updateBackendFailed: '更新后端失败',
      deleteBackendFailed: '删除后端失败',
      fetchGlossaryFailed: '获取术语表失败',
      createGlossaryEntryFailed: '创建术语条目失败',
      updateGlossaryEntryFailed: '更新术语条目失败',
      deleteGlossaryEntryFailed: '删除术语条目失败',
      importGlossaryFailed: '导入术语表失败',
      exportGlossaryFailed: '导出术语表失败',
      fetchPromptTemplatesFailed: '获取提示词模板列表失败',
      fetchPromptTemplateFailed: '获取提示词模板详情失败',
      createPromptTemplateFailed: '创建提示词模板失败',
      updatePromptTemplateFailed: '更新提示词模板失败',
      deletePromptTemplateFailed: '删除提示词模板失败',
      fetchBootstrapPromptTemplatesFailed: '获取术语抽取模板列表失败',
      fetchBootstrapPromptTemplateFailed: '获取术语抽取模板详情失败',
      createBootstrapPromptTemplateFailed: '创建术语抽取模板失败',
      updateBootstrapPromptTemplateFailed: '更新术语抽取模板失败',
      deleteBootstrapPromptTemplateFailed: '删除术语抽取模板失败',
      fetchPrunePromptTemplatesFailed: '获取术语精简模板列表失败',
      fetchPrunePromptTemplateFailed: '获取术语精简模板详情失败',
      createPrunePromptTemplateFailed: '创建术语精简模板失败',
      updatePrunePromptTemplateFailed: '更新术语精简模板失败',
      deletePrunePromptTemplateFailed: '删除术语精简模板失败',
      fetchExecutionProfilesFailed: '获取执行策略列表失败',
      fetchExecutionProfileFailed: '获取执行策略详情失败',
      createExecutionProfileFailed: '创建执行策略失败',
      updateExecutionProfileFailed: '更新执行策略失败',
      deleteExecutionProfileFailed: '删除执行策略失败',
      fetchExecutionPlanTemplatesFailed: '获取执行计划模板列表失败',
      fetchExecutionPlanTemplateFailed: '获取执行计划模板详情失败',
      createExecutionPlanTemplateFailed: '创建执行计划模板失败',
      updateExecutionPlanTemplateFailed: '更新执行计划模板失败',
      deleteExecutionPlanTemplateFailed: '删除执行计划模板失败',
      fetchSegmentGroupsFailed: '获取章节分组失败',
      previewSegmentTranslationFailed: '生成单段试译预览失败',
      previewSegmentRevisionFailed: '生成单段修订预览失败',
      applySegmentTranslationPreviewFailed: '应用单段试译结果失败',
      previewSearchReplaceFailed: '生成搜索替换预览失败',
      applySearchReplaceFailed: '应用搜索替换失败',
      undoSearchReplaceFailed: '撤销搜索替换失败',
      qaRecheckFailed: '执行 QA 重检失败',
      glossarySyncImpactFailed: '术语同步影响分析失败',
      glossarySyncExecuteFailed: '术语同步任务提交失败',
      glossarySyncStatusFailed: '术语同步任务状态查询失败',
      glossarySyncCancelFailed: '术语同步任务取消失败',
      glossaryPrunePreviewFailed: '生成术语精简建议失败',
      glossaryPruneApplyFailed: '应用术语精简建议失败',
      fetchAdminStatsFailed: '获取系统统计失败',
      fetchAdminUsersFailed: '获取用户列表失败',
      fetchAdminUserFailed: '获取用户详情失败',
      createAdminUserFailed: '创建用户失败',
      updateAdminUserFailed: '更新用户失败',
      disableAdminUserFailed: '停用用户失败',
      resetAdminUserPasswordFailed: '重置密码失败',
      fetchAdminAuditLogsFailed: '获取审计日志失败',
      fetchAdminSettingsFailed: '获取系统设置失败',
      updateAdminSettingsFailed: '更新系统设置失败',
      quickTranslateFailed: '即时翻译失败',
    },
  },
} as const

export default messages
