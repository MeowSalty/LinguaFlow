...
export interface paths {
    ...
    "/projects/{projectId}/resources/{resourceId}/download-translated": {
         ...
         *     渲染前会预检译文标签结构：存在无法原样嵌入导出文档的译文时（如标签未闭合）
         *     返回 409 Conflict，detail 给出缺陷段落编号与原因。此时不写出任何文件内容，
         *     避免把「该章整章回退为原文」的残缺结果静默交付给用户。
         */
...
export interface components {
    schemas: {
        ...
        /**
         * @description 统一思考强度档位（手动开关式）。不设置该键 = 开关关闭，不传任何 thinking 字段，
        ...
        SearchReplaceSkippedItem: {
            segment_id: number;
            /**
             * @description 跳过原因；no_longer_matches=apply 时当前译文已不含匹配，empty_result=替换后译文为空，target_diverged=undo 时该段已被后续编辑，invalid_markup=替换后译文标签结构非法（会导致导出时整章降级为原文）
             * @enum {string}
             */
            reason: "no_longer_matches" | "empty_result" | "target_diverged" | "invalid_markup";
        };
