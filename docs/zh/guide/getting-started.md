...
::: tip 只想译一段，不建项目？
启动后的首页本身就是一个 [即时翻译](/zh/guide/quick-translate) 框。只要按本页第 2、3 步配好后端与执行计划，就能直接粘贴文本出译文，不必走项目 / 资源 / 作业的完整流程。
:::
...
1. 在资源列表勾选刚上传的文件
2. 点击浮动操作栏的 **处理选中**（或点击资源进入编辑视图后从工具栏发起）
3. 在面板中选择第 3 步的执行计划
4. 其余选项可保持默认（首次不必开「自动审批」）
5. 点击 **开始翻译**
6. 在 **任务** 标签查看进度；完成后点击资源进入编辑视图查看原文与译文
...
| 目标                               | 文档                                                                                 |
| ---------------------------------- | ------------------------------------------------------------------------------------ |
| 不建项目、只译一段                 | [即时翻译](/zh/guide/quick-translate)                                                |
