...
译文中存在同类问题（术语不统一、多余空格、标点风格不一致等）时，不必逐段编辑：在段落面板工具栏点 **搜索替换**，在滑出的抽屉中对当前资源的**译文**批量查找替换，原文不受影响。

### 操作流程

1. 输入查找内容与替换文本（替换文本留空表示删除匹配内容）
2. 按需调整匹配选项：
   - **匹配方式** — 字面子串（默认）或正则（RE2 语法，替换文本支持 `$1` 捕获引用）
   - **区分大小写** — 默认开启
   - **全字匹配** — 只匹配完整词汇，不含子串命中；仅子串模式生效，正则模式下可用 `\b` 边界替代
...
在段落列表顶部的搜索框中输入关键词，可以筛选出包含该内容的段落。输入搜索词后，还可以进一步控制匹配范围：

- **字段范围** — 下拉选择「原文+译文」（默认）/「仅原文」/「仅译文」
- **区分大小写** — 「Aa」按钮切换
- **结果计数** — 工具栏显示「已加载 N / 共 M 段」，方便确认是否还有更多结果
