...
server:
  ...
  pipeline:
    max_inflight_weight_mb: 32   # 每任务在途工作配额（源文本字节，MB）
    max_inflight_resources: 8    # 每任务在途资源数上限
    rss_limit_mb: 0              # 进程 RSS 保险丝；0 = 关闭
  quick_translate:
...
| 字段                    | 类型 | 默认值 | 说明                                                                                                          |
| ----------------------- | ---- | ------ | ------------------------------------------------------------------------------------------------------------- |
| `max_inflight_weight_mb` | int  | `32`   | 每任务同时在途的源文本字节配额（MB）。这是**并发节流**而非内存上限：超出配额的资源排队等待，超预算的单资源在配额空置时独跑放行，不会饥饿。`<=0` 用默认值 |
| `max_inflight_resources` | int  | `8`    | 每任务同时在途的资源数上限，兜住每资源的句柄开销。`<=0` 用默认值                                                |
...
控制首页「即时翻译」（同步单段在线翻译）的并发与超时。译文纯临时、不落库，故无 `apply_token_ttl`。功能说明见 [即时翻译](/zh/guide/quick-translate)。
...
控制作业实时事件（SSE）的回放与历史事件存储行为，影响 [进度追踪](/zh/guide/projects#进度追踪) 与事件时间线的首字节延迟与历史补进。任务级进度事件（暂停、恢复等）也经此通道推送。
