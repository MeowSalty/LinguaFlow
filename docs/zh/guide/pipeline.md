...
任务执行按「资源 × 轮次」推进，每个轮次内已完成的段落会**即时落库**为断点：

- **暂停**（优雅排空）— 停止派发新批次，等在途 LLM 请求返回并落库后冻结任务；资源与轮次保持「运行中」可见，进度保留真实比例
- **恢复** — 任务重新入队，从轮次断点继续：已完成的轮次/段落直接跳过，已完成计数保留并继续累加
- **失败/取消后重试** — 同样从断点继续；唯一例外是显式手选段落的任务，首个翻译轮会重译全部选中段落
- 暂停与取消走的是**相互独立的两条通路**：取消是终态判定，暂停只是可恢复的闸门——内存高水位时系统内部的「新资源准入暂停」也复用同一闸门机制，任务状态保持运行中（见 [配置文件与环境变量 · server.pipeline](/zh/guide/configuration#server-pipeline-—-流水线准入)）

界面操作与轮次矩阵呈现见 [项目管理 · 暂停与恢复](/zh/guide/projects#暂停与恢复)。
...
翻译轮次内由 QA 引擎同步跑规则检查并写回段落。共 19 项 per-batch checker（可在执行配置 `qa.checks` 中按名选择性启用）+ 1 项文档级 `duplicate_source_divergence`（跨段比对，不可在 `qa.checks` 中关闭）；另由翻译轮的注音守恒逻辑产出 `ruby_restore_incomplete` / `ruby_tag_loss` 两个守恒码：
...
| `script_mismatch`             | 多文字系统语言（zh-Hans/zh-Hant 等）译文混入兄弟文字系统字符，出现即报；单文字系统语言与 `auto` 不活跃 | ❌ 硬规则    |
...
| `xml_tag_mismatch`           | XML 标签集合不一致，或译文 XML 结构损坏（标签未闭合/嵌套错误） | ❌ 硬规则    |

源语残留按 Unicode 脚本与语言对分档（独立脚本偏严；共汉字语言对有不同策略）。源语言为 `auto` 时残留检测不生效。`untranslated` 同样语言对感知：译文含目标语不使用的源语独有脚本时报 `error`，共用文字系统降为 `warning`，比较前统一剥离 Ruby 注音并豁免纯占位符内容。规则与可裁决性等细节见 [翻译审校 · 质量检测](/zh/guide/review#质量检测)。
...
::: tip XML 标签与 Ruby 注音
通用的 `xml_tag_mismatch` 比对源/译 XML 标签多重集时，会**排除 `<ruby>`/`<rt>`/`<rp>`/`<rb>` 注音标签族**——这些标签的守恒由 Ruby 还原子系统独立负责（`preserve_kinds` 过滤会合法地从译文移除它们），在通用标签比对里排除能避免配置耦合产生的误报。但排除只针对标签集合比对，注音标签自身的未闭合或嵌套错误仍会被结构检查发现。
:::

### 译文结构守卫（EPUB）

EPUB 的译文最终要嵌回原书的 XHTML。为保证导出的电子书能被正常打开，流水线对 EPUB 译文实施**全链路结构守卫**：

- **翻译/校对/修订写回时** — AI 生成的译文若是结构退化的 XML 片段（标签未闭合、嵌套交错等），该次改写被拒绝、不落库，沿用既有的失败处理路径（重试/回退）；翻译结果也不会进入术语记忆，避免坏译文扩散
- **导出渲染容错** — 单段译文结构非法时只跳过该段、保留对应元素的原文，**不会**再让整章译文回退为原文；原 XHTML 本身含非严格 XML 内容时，已生成的译文也会被保留
- **导出兜底** — 下载前对全部译文做结构预检，存在缺陷时不输出文件并返回错误明细，见 [翻译审校 · 结构预检](/zh/guide/review#结构预检)

其他格式（TXT 等）没有标签结构，不做此校验。
...
1. **译前** 抽出注音，正文只留 base。单个 `<ruby>` 内可含多组 `<rt>`（如 `<ruby>何<rt>な</rt>故<rt>ぜ</rt></ruby>`），按「基底—注音」逐对提取并统一剥离全部注音，送翻文本不残留注音文字；同一元素内及相邻元素间的单汉字注音会合并为词级条目
2. **译后** 按类型过滤，把标签插回译文
