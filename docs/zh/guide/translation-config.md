...
::: warning 批次限制
翻译 / 提取 / 裁决轮次中，`batch_size` 和 `max_words_per_batch` 至少填一项（不能同时为 0）。
:::
...
flowchart TD
    ...
    subgraph Round[Round 1..N 按顺序执行]
        ModeSwitch{round.mode}
        ModeSwitch -->|translate| TranslatePipe[翻译流水线<br/>Protect → Split → Context → Translate → QA → Postprocess]
        ModeSwitch -->|extract| ExtractPipe[术语提取]
        ModeSwitch -->|adjudicate| AdjudicatePipe[质量裁决<br/>对已有 quality_issues 逐条复核]
        TranslatePipe --> QA[规则质检写入 quality_issues]
    end

    Round --> RubyCheck{ruby_retry.enabled?}

    RubyCheck -->|是| RubyRetry[Ruby Retry 阶段]
    RubyCheck -->|否| Done([完成])

    RubyRetry --> Done
```

典型推荐顺序：`extract`（可选）→ `translate`（可多轮回退）→ `adjudicate`（可选降噪）→ Ruby 重试（可选）。
