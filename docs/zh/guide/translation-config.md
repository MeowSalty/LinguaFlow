...
| **执行计划**   | 将后端、模板、配置组合成模式化多轮翻译流水线（翻译轮次 + 提取轮次）          |
...
| 概念                      | 说明                                                               |
| ------------------------- | ------------------------------------------------------------------ |
| **翻译轮次（translate）** | 翻译流水线中的一个翻译阶段，引用特定的后端 + 提示词模板 + 执行配置 |
| **提取轮次（extract）**   | 可选的术语提取阶段，引用术语抽取模板，从源文本中提取候选术语       |
...
独立的术语提取阶段，在翻译前执行。使用独立的术语抽取提示词模板。

| 字段                  | 类型 | 默认值  | 说明                      |
| --------------------- | ---- | ------- | ------------------------- |
| `enabled`             | bool | `false` | 是否启用独立术语提取      |
| `backend_id`          | int  | `0`     | 术语提取使用的后端 ID     |
| `prompt_template_id`  | int  | `0`     | 术语抽取提示词模板 ID     |
| `batch_size`          | int  | `20`    | 每批发送给 LLM 的源文段数 |
| `concurrency`         | int  | `2`     | 提取并发数                |
| `max_terms_per_batch` | int  | `20`    | 每批最多抽取的术语数      |
| `min_source_len`      | int  | `2`     | 术语源文最短字符数        |

#### 轮次配置（Rounds）

每个轮次是一个模式化执行阶段，分为翻译轮次和提取轮次：

| 字段                  | 类型   | 说明                                                          |
| --------------------- | ------ | ------------------------------------------------------------- |
| `mode`                | string | 轮次模式：`translate`（翻译）或 `extract`（术语提取）         |
| `backend_id`          | int    | 引用的后端 ID                                                 |
| `prompt_template_id`  | int    | 引用的提示词模板 ID（翻译轮次用翻译模板，提取轮次用抽取模板） |
| `profile_id`          | int    | 引用的执行配置 ID（仅翻译轮次）                               |
