...
推荐顺序：`extract`（可选）→ `translate`（可多轮）→ `adjudicate`（可选）→ `semantic_qa`（可选）。
...
裁决与语义质检可以叠加使用：裁决只对已有规则问题做「保留/剔除」，语义质检则会**新增**语义类问题，二者不冲突。
...
::: info 裁决提示词
质量裁决与语义质检均使用系统内置提示词，**不能**选自定义模板。`untranslated` / `duplicate` 为硬规则，不可裁决。
:::
