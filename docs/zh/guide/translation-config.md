...
LinguaFlow 提供灵活的翻译配置系统，让您精确控制翻译行为。本文档涵盖四个核心配置模块：

| 模块             | 说明                                     |
| ---------------- | ---------------------------------------- |
| **AI 后端**      | 翻译使用的 AI 服务及其连接参数           |
| **提示词模板**   | 发送给 AI 的翻译指令（Go template 语法） |
| **翻译配置文件** | 翻译行为的详细参数（分段、保护、修复等） |
| **执行计划**     | 将后端、模板、配置组合成多轮翻译流水线   |
...
| 模板                 | 字段名                     | 用途                         |
| -------------------- | -------------------------- | ---------------------------- |
| **翻译提示词**       | `system_prompt_content`    | 翻译任务的 system prompt     |
| **Bootstrap 提示词** | `bootstrap_prompt_content` | 术语抽取任务的 system prompt |

### 内置模板

LinguaFlow 提供内置的默认提示词模板（scope 为 `system`，ID 为 `-1`），适用于大多数场景。内置模板不可修改或删除。
...
翻译配置文件（Translation Profile）控制翻译的详细行为，包括分段、保护、修复等参数。
...
执行计划（Execution Plan Template）将多个 AI 后端、提示词模板和翻译配置组合成多轮翻译流水线。

### 核心概念

| 概念              | 说明                                                           |
| ----------------- | -------------------------------------------------------------- |
| **轮次（Round）** | 翻译流水线中的一个阶段，引用特定的后端 + 提示词模板 + 翻译配置 |
| **Bootstrap**     | 可选的前置阶段，用于从源文本中自动抽取术语                     |
| **Ruby 重试**     | 可选的后置阶段，用于修复注音对齐问题                           |

### 使用场景

- **失败重试** — 第一轮翻译失败的段落由后续轮次使用不同模型/配置重试
- **多模型对比** — 使用不同模型翻译，人工选择最佳结果
- **质量检查** — 翻译后使用另一个模型进行质量检查
- **术语自举** — 先从源文本抽取术语，再进行翻译
...
| 字段                  | 类型 | 默认值  | 说明                                                         |
| --------------------- | ---- | ------- | ------------------------------------------------------------ |
| `enabled`             | bool | `false` | 是否启用独立自举                                             |
| `backend_id`          | int  | `0`     | 自举使用的后端 ID                                            |
| `prompt_template_id`  | int  | `0`     | 自举使用的提示词模板 ID（仅用其 `bootstrap_prompt_content`） |
| `batch_size`          | int  | `20`    | 每批发送给 LLM 的源文段数                                    |
| `concurrency`         | int  | `2`     | 自举并发数                                                   |
| `max_terms_per_batch` | int  | `20`    | 每批最多抽取的术语数                                         |
| `min_source_len`      | int  | `2`     | 术语源文最短字符数                                           |
...
| 字段                  | 类型   | 说明                                |
| --------------------- | ------ | ----------------------------------- |
| `name`                | string | 轮次名称（如 "主翻译"、"备用模型"） |
| `backend_id`          | int    | 引用的后端 ID                       |
| `prompt_template_id`  | int    | 引用的提示词模板 ID                 |
| `profile_id`          | int    | 引用的翻译配置 ID                   |
| `batch_size`          | int    | 每批段落数上限，`0` 不限制          |
| `max_words_per_batch` | int    | 每批字词数上限，`0` 不限制          |
| `concurrency`         | int    | 并发翻译数（≥ 1）                   |
| `fallback_shrink`     | float  | 整批失败时的缩放因子（0, 1）        |
| `retry`               | object | 重试配置（见下方）                  |

::: warning 批次限制
`batch_size` 和 `max_words_per_batch` 至少填一项（不能同时为 0）。
:::
...
执行计划在翻译任务创建时生成快照，然后由 Worker 异步执行：

```mermaid
flowchart TD
    Start([翻译任务创建]) --> BootstrapCheck{bootstrap.enabled?}
    
    BootstrapCheck -->|是| Bootstrap[Bootstrap 阶段]
    BootstrapCheck -->|否| Round
    
    subgraph Bootstrap[Bootstrap 阶段]
        B1[使用 bootstrap.backend + bootstrap.prompt]
        B2[按 batch_size 分批发送源文本]
        B3[LLM 抽取术语 → 写入术语表]
        B1 --> B2 --> B3
    end
    
    Bootstrap --> Round
    Start -->|跳过 Bootstrap| Round
    
    subgraph Round[Round 1..N 按顺序执行]
        R1[使用 round.backend + round.prompt + round.profile]
        
        subgraph Pipeline[Pipeline Stage 顺序]
            P1[1. Protect<br/>内容保护 → 占位符替换]
            P2[2. Split<br/>按 profile.split 分段]
            P3[3. Context<br/>按 profile.context 添加上下文]
            P4[4. Translate<br/>分批并发翻译]
            P5[5. Postprocess<br/>后处理]
            P6[6. Unprotect<br/>还原占位符]
            P7[7. RubyRestore<br/>注音还原]
            P1 --> P2 --> P3 --> P4 --> P5 --> P6 --> P7
        end
        
        subgraph TranslateDetail[Translate 阶段详情]
            T1[batch_size / max_words_per_batch]
            T2[concurrency 并发控制]
            T3[retry 重试策略]
            T4[fallback_shrink 缩放降级]
            T5[repair 响应修复]
        end
        ...
        GlossaryCheck{glossary.bootstrap.enabled?}
        GlossaryCheck -->|是| InlineBootstrap[内联 Bootstrap<br/>翻译同时抽取术语]
    end
    
    Round --> RubyCheck{ruby_retry.enabled?}
    
    RubyCheck -->|是| RubyRetry[Ruby Retry 阶段<br/>使用 ruby_retry.backend 重试注音对齐]
    RubyCheck -->|否| Done([翻译完成])
...
::: info 内置资源
LinguaFlow 提供的默认提示词模板和翻译配置使用 `system` 作用域，用户不可修改或删除，但可以基于它们创建自定义副本。
:::
...
1. **使用术语表** — 确保专业术语翻译一致，可通过 Bootstrap 自动抽取或手动添加
