...
| **提示词模板** | 发给模型的指令模板（翻译 / 术语提取 / 术语精简） | 模板与配置 · 提示词 |
| **执行配置** | 分段、内容保护、修复、质检、上下文等行为参数 | 模板与配置 · 执行策略 |
| **执行计划** | 多轮流水线：每轮用哪个后端、什么模式（翻译 / 提取 / 本地改写 / LLM 修订 / 质量裁决 / 语义质检），顶层引用执行配置 | 模板与配置 · 执行计划 |
| **即时翻译** | 单段文本临时翻译，复用执行计划与术语表，**不落库** | 首页翻译控制台 |
...
::: tip 三种翻译入口怎么选
- **即时翻译** — 单段临时译，不建项目、不留底，详见 [即时翻译](/zh/guide/quick-translate)
- **项目翻译** — 批量、可审校、可导出，本页其余概念主要服务于它
- **单段预览** — 在项目段落上试效果（试译出全新译文 / 修订对现有译文定点修订），满意才「应用译文」写回，见 [翻译配置 · 使用 · 单段试译](/zh/guide/translation-config#单段试译) / [单段修订预览](/zh/guide/translation-config#单段修订预览)
:::
...
1. 用哪一个 AI 后端调用模型  
2. 用哪套提示词与执行配置（执行配置在计划顶层引用，translate 与 revise 轮共享）  
3. 是否在翻译前抽术语、翻译后做本地改写 / 质量裁决 / LLM 修订 / 语义质检  

**第一次使用**：只建「单轮翻译 + 内置通用提示词/策略」即可，见 [快速开始 · Web](/zh/guide/getting-started)。

进阶组合（提取 → 翻译 → 裁决 → 语义质检）见 [翻译配置 · 使用](/zh/guide/translation-config)。想对单段先试效果再看要不要应用，用 [单段试译](/zh/guide/translation-config#单段试译)。
...
2. 本页（建立地图）  
3. [即时翻译](/zh/guide/quick-translate)（只想译一段时） · [项目管理](/zh/guide/projects) → [术语表](/zh/guide/glossary) → [审校](/zh/guide/review)  
