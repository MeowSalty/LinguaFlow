...
// QueryReviewedSegments chains the current query on the "reviewed_segments" edge.
func (_q *UserQuery) QueryReviewedSegments() *SegmentQuery {
...
// WithReviewedSegments tells the query-builder to eager-load the nodes that are connected to
// the "reviewed_segments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithReviewedSegments(opts ...func(*SegmentQuery)) *UserQuery {
	query := (&SegmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withReviewedSegments; query != nil {
...
func (_q *UserQuery) loadReviewedSegments(ctx context.Context, query *SegmentQuery, nodes []*User, init func(*User), assign func(*User, *Segment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Segment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReviewedSegmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_reviewed_segments
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_reviewed_segments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadOwnedProjects(ctx context.Context, query *ProjectQuery, nodes []*User, init func(*User), assign func(*User, *Project)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(project.FieldOwnerUserID)
	}
