...
func TestProtocolFromResponseMode(t *testing.T) {
	cases := []struct {
		mode string
		want Protocol
	}{
		{"", ProtocolJSONStrict},
		{"json_schema", ProtocolJSONStrict},
		{"json_object", ProtocolJSONLoose},
		{"none", ProtocolJSONLoose},
		{"text", ProtocolText},
		{"unknown", ProtocolJSONLoose},
	}
	for _, tc := range cases {
		if got := ProtocolFromResponseMode(tc.mode); got != tc.want {
			t.Errorf("ProtocolFromResponseMode(%q)=%q, want %q", tc.mode, got, tc.want)
		}
	}
}

func TestRenderer_ProtocolJSONShape(t *testing.T) {
	...
	if strings.Contains(sysStrict, "输出 JSON 结构如下") {
		t.Errorf("json_strict should omit shape example:\n%s", sysStrict)
	}
	...
	if !strings.Contains(sysLoose, "输出 JSON 结构如下") {
		t.Errorf("json_loose should include shape example:\n%s", sysLoose)
	}
}
...
func TestRenderer_TextModeWithContext(t *testing.T) {
	r, err := NewRenderer(`{{- if eq .Protocol "text"}}text mode{{- else}}json mode{{- end}}`)
	if err != nil {
		t.Fatalf("renderer: %v", err)
	}
	data := Data{
