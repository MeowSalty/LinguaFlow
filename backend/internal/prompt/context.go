...
import "github.com/MeowSalty/LinguaFlow/backend/internal/pipeline"

// BuildContext 从 Document 中提取目标段落的前/后段作为上下文。
// 优先返回 OriginalSource（protect 前的原文），让 LLM 看到可读上下文而非 __LF_xxxx__。
// idx 越界返回空字符串。
func BuildContext(doc *pipeline.Document, idx int) (prev, next string) {
...
func BuildContextRange(doc *pipeline.Document, from, to int) (prev, next string) {
	if from > 0 {
...
func contextText(s *pipeline.Segment) string {
	if s.OriginalSource != "" {
