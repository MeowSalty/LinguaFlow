...
- **质量检测**：
  ...
  - 本地改写：纯本地机械修复译文丢失引号等高频安全问题，不调 LLM
...
规则质检偏敏感，难免误报。质量裁决轮次（`adjudicate`）调用 AI 对已标出的问题逐条复核，将明确误报标记为「已驳回」（保留记录，不再计入筛选与修复），实现质检降噪；也可在审校界面对单条质量问题人工驳回或撤销裁决。可裁决 `source_residual`（源语残留）与 `length_ratio`（长度异常）两类软规则；`untranslated`（未翻译）与 `duplicate`（重复翻译）为硬规则，不可裁决。

裁决只对已有规则问题做"保留/驳回"、不新增问题；若需捕获规则查不出的语义错误（逐字借译、漏译、增译、语法错误、语体错位等），可在执行计划中再加一轮**语义质检**（`semantic_qa`），产出的语义类问题带文本区间高亮、直接进人审，二者可叠加使用。对译文丢失引号等可确定修复的安全问题，可加一轮**本地改写**（`correct`），纯本地机械改写、不调 LLM；对语义质检/裁决产出但未决的语义问题，可加一轮**LLM 修订**（`revise`），调 LLM 对现有译文做定点最小修订，系统提示词内置。三者都与裁决、语义质检互补。
