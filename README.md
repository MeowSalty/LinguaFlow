...
- 创建、编辑、删除项目
- 内置 12 种语言预设（英、简中、繁中、日、韩、西、法、德、葡、俄、阿拉伯、意大利）互译，源语言支持自动识别
- 项目级别的术语表管理
...
- 单段修订预览：以段落上待处理的语义问题为修复目标，对现有译文做定点最小修订，预览满意后再应用写回

### 批量处理

一次翻译整个目录的文件，支持增量更新；每个文件内部可按段落分批并发调用模型。

- 拖拽上传文件或文件夹（多文件合并为批次任务，按文件数汇总进度）
- 增量更新：保留已有译文，仅翻译新增内容
...
- **EPUB**：编辑视图左侧章节侧栏按章节浏览与翻译、章节级进度、HTML 内容预览，支持章节级批量翻译 / QA 重检；译文全程校验 XML 结构合法性，保证导出的电子书始终可正常打开
...
- 搜索定位（Ctrl+F）：跨章节搜索原文 / 译文，支持正则、全字、大小写选项，结果一键跳转并在正文高亮
- 搜索替换：对译文批量查找替换（子串/正则、区分大小写、全字匹配），先预览影响范围再应用，可撤销
- 质量问题人工驳回：判定不是问题的可标记驳回（可撤销），已驳回问题不再计入筛选与自动修复
  - **质量检测**：
  - 规则质检（19 项确定性 checker）：长度异常、重复、未翻译、源语残留、标点配对、标点缺失、标点多出、外层引号包裹丢失、异常空白、全/半角混用、文字系统混用（如简繁混排）、数字/URL/邮箱/字幕行数不一致、禁译/强制术语命中、占位符残留、XML 标签不一致、跨段同源异译等；另由翻译轮产出注音守恒警告
  - 语义质检（LLM）：捕获规则查不出的语义错误（逐字借译、术语误译、漏译、增译、语法错误、语体错位等），结果带文本区间高亮、直接进人审
  - 本地改写：纯本地机械修复译文丢引号、全/半角混用等高频安全问题，不调 LLM
  - LLM 修订：调 LLM 对现有译文按待处理语义问题做定点最小修订，系统提示词内置
  - AI 质量裁决：对软规则误报逐条复核降噪（见下）
  - QA 重检：按执行配置当前质检设置对既有译文重跑 QA，继承人工裁决、不动译文
...
- **执行计划**：组合后端、提示词、配置，定义模式化多轮翻译流程（翻译、提取、本地改写、LLM 修订、质量裁决、语义质检轮次），顶层引用执行配置

### 质量裁决与语义质检

规则质检偏敏感，难免误报。质量裁决轮次（`adjudicate`）调用 AI 对已标出的问题逐条复核，将明确误报标记为「已驳回」（保留记录，不再计入筛选与修复），实现质检降噪；也可在审校界面对单条质量问题人工驳回或撤销裁决。可裁决 `source_residual`（源语残留）、`punctuation_surplus`（标点多出）、`length_ratio`（长度异常）与 `untranslated`（未翻译）等软规则——未翻译检测按语言对分级（源/目标语共用文字系统时降为警告），专名、品牌等合理原样保留可由裁决复核；`duplicate`（重复翻译）为硬规则，不可裁决。

裁决只对已有规则问题做"保留/驳回"、不新增问题；若需捕获规则查不出的语义错误（逐字借译、漏译、增译、语法错误、语体错位等），可在执行计划中再加一轮**语义质检**（`semantic_qa`），产出的语义类问题带文本区间高亮、直接进人审，二者可叠加使用。对译文丢引号、全/半角混用等可确定修复的安全问题，可加一轮**本地改写**（`correct`），纯本地机械改写、不调 LLM；对语义质检/裁决产出但未决的语义问题，可加一轮**LLM 修订**（`revise`），调 LLM 对现有译文做定点最小修订，系统提示词内置。三者都与裁决、语义质检互补。
