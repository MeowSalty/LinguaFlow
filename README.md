...
规则质检偏敏感，难免误报。质量裁决轮次（`adjudicate`）调用 AI 对已标出的问题逐条复核，将明确误报标记为「已驳回」（保留记录，不再计入筛选与修复），实现质检降噪；也可在审校界面对单条质量问题人工驳回或撤销裁决。可裁决 `source_residual`（源语残留）、`punctuation_surplus`（标点多出）与 `length_ratio`（长度异常）等软规则；`untranslated`（未翻译）与 `duplicate`（重复翻译）为硬规则，不可裁决。
